...
| 36-04-01 | 04 | 4 | SYS-02, EVD-11, EVD-12, EVD-14, EVD-15 | all | Reconciliation occurs only after clean verification | repository | `just test && just parity && just verify-reference && just verify-redaction` | ✅ | ✅ passed at `f54b69c3cf5f77792aa4be10080a0e86b3457ba4`: 73/73; no parity errors; reference clean; redaction passed (fresh 2026-07-24) |
...
Fresh verification on 2026-07-24 used source commit `f54b69c3cf5f77792aa4be10080a0e86b3457ba4`. No canonical planning-truth reconciliation was performed.

| Gate | Observed result |
| --- | --- |
| Mandatory Rust sequence | `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo build --all-targets --all-features`, and `cargo test --all-features` passed in the required order with fresh isolated `CARGO_TARGET_DIR=/tmp/bitaxe-phase36-iteration4-gate.8otbNa`; the parity crate ran 383 tests. |
...
Five Task 1/review gate failures were fixed before the final fresh run: commit `ff46c450` gives concurrent generation tests collision-resistant workspace identities, commit `857e2a95` makes Bazel declare and pass the exact ESP-IDF packaging inputs instead of relying on a source-tree cache, commit `37b7bc47` authenticates immutable evidence artifacts at the authority boundary, commit `cc4784ca` binds transitive evaluator validators into provenance, and commit `f54b69c3` binds the runtime replay validator into that inventory.
