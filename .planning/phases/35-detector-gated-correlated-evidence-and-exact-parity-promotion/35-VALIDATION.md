...
| --- | --- | --- | --- |
| Exact-current-package Ultra 205 evidence root | CFG-12, EVD-10–EVD-15 | Requires the physically connected board and one approved normal reboot | Attempts 19–27 remain sealed. Attempt 27 used the OS-native device-session contract and stopped before restart with `observer_unqualified`; protected diagnosis isolated a deterministic nested-ioreg property parser defect while preserving the detector-bound physical identity. Attempt 28 is authorized only after the sanitized regression, parser fix, complete software gate, commit, and exact-head preflight pass. Its authoritative reboot proof requires the same physical device, a changed HTTP boot session, exact `N → N+1` ordinal, `software_cpu`, exact build identity, and the persisted hostname; serial output, USB re-enumeration, and sampled service loss are corroborating only. Task 2 and Phase 35 remain incomplete without eligible evidence. |
