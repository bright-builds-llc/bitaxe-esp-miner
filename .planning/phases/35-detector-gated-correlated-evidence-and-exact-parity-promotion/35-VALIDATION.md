...
| --- | --- | --- | --- |
| Exact-current-package Ultra 205 evidence root | CFG-12, EVD-10–EVD-15 | Requires the physically connected board and one approved normal reboot | Attempts 19–30 remain immutable non-promotions. Attempt 30 passed the complete hardware, restoration, cleanup, and typed-root validation chain, but the redaction gate rejected its redundant full-checklist publication. Attempt 31 is authorized only after the narrow publisher and added-line scanner regressions, repair commit, complete software gate, and exact-head preflight pass. Its authoritative reboot proof remains the same physical device, changed HTTP boot session, exact `N → N+1` ordinal, `software_cpu`, exact build identity, and persisted hostname; serial output, USB re-enumeration, and sampled service loss are corroborating only. Task 2 and Phase 35 remain incomplete without commit-safe eligible evidence. |
