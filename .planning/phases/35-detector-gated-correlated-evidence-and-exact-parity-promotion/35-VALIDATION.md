...
| --- | --- | --- | --- |
| Exact-current-package Ultra 205 evidence root | CFG-12, EVD-10–EVD-15 | Requires the physically connected board and one approved normal reboot | Attempts 19–29 remain sealed. Attempt 29 passed the complete authoritative reboot quorum, restoration, and cleanup, then the evidence validator rejected newline-added epoch artifact bytes as `inventory_mismatch`. Attempt 30 is authorized only after all six epoch artifacts have a byte-exact producer regression, the repair is committed, and the complete software gate and exact-head preflight pass. Its authoritative reboot proof remains the same physical device, a changed HTTP boot session, exact `N → N+1` ordinal, `software_cpu`, exact build identity, and the persisted hostname; serial output, USB re-enumeration, and sampled service loss are corroborating only. Task 2 and Phase 35 remain incomplete without eligible evidence. |
