...
- **D-44:** Attempt 30 proves the byte-exact producer repair and completes the authoritative hardware chain, restoration, cleanup, inventory, and typed-root validation. Before commit, `verify-redaction` correctly rejected the candidate generation because the atomic publisher copied the complete historical checklist into the admitted-evidence tree, duplicating legacy protected-operational values. The candidate promotion was rolled back. The repaired publisher keeps the canonical checklist as the sole checklist artifact, fingerprints and atomically updates it, and admits only projection, matrix, verdict, and manifest; the staged scanner now evaluates added lines while full admitted-tree scanning remains unchanged. Attempt 31 may run only after this checkpoint, a clean commit, the complete software gate, and exact-head preflight pass. Attempt 30 remains immutable, non-promotable, non-reusable, and ineligible for splicing.
