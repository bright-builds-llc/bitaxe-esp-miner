...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 30-01-01 | 01 | 1 | STR-09, CFG-07, ASIC-11 | T-30-01 | No-input disposition cannot be interpreted as promotion or live proof | static/regression | `rg -n "phase30_disposition: no_promotion_no_eligible_evidence|new_evidence_input: none|eligible_share_outcome: none" docs/parity/evidence/phase-30-live-share-outcome-and-verified-promotion/disposition.md` | ❌ W0 | ⬜ pending |
...
| 30-02-01 | 02 | 2 | STR-09, CFG-07, ASIC-11 | T-30-03 | Verified rows require explicit row-specific eligible Phase 30 evidence | unit/regression | `cargo test -p bitaxe-parity --all-features phase30_ && bazel test //tools/parity:tests` | ❌ W0 | ⬜ pending |
| 30-02-02 | 02 | 2 | STR-09, CFG-07, ASIC-11 | T-30-04 | Final closure retains pending traceability and every explicit non-claim | repository gate | `just parity && just verify-reference && git diff --check` | ✅ | ⬜ pending |
...
- [ ] Add focused `phase30_` regression fixtures in `tools/parity/src/main.rs` before changing the guard: conservative rows pass; fabricated verified STR-09, CFG-07, and ASIC-11 rows fail without exact row-specific admission.
- [ ] Add a failing artifact-contract check for the absent `docs/parity/evidence/phase-30-live-share-outcome-and-verified-promotion/disposition.md` before creating it.
- [ ] Preserve the existing Phase 28 promotion tests as regression coverage; do not replace or weaken them.
- [ ] Record the exact Cargo package (`bitaxe-parity`) and Bazel target (`//tools/parity:tests`) in each implementation task.
...
- [ ] Set `wave_0_complete: true` only after the test-first red/green steps execute.
