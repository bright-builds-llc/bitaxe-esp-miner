...
**Status:** Shipped with accepted unresolved gaps
**Shipped:** 2026-07-13
**Phases:** 22–30, including inserted Phases 28.1 and 28.1.1–28.1.1.7
**Plans:** 76
**Requirements:** 18/21 satisfied; STR-09, ASIC-11, and CFG-07 remain pending

This archive preserves the complete roadmap and diagnostic history. “Shipped” records administrative milestone closure; it does not claim that live ASIC nonce correlation, a live share outcome, or eligible credential-handling evidence was verified.
...
- Exact claim-ladder and safety-prerequisite governance with blocker reasons propagated through runtime and API surfaces.
- A production-oriented BM1366 and Stratum software chain with bounded safe-stop, telemetry projection, redacted evidence contracts, and atomic evidence consolidation.
- Hardware-backed isolation of the remaining Rust firmware nonce-production blocker while upstream firmware mined successfully on the same Ultra 205.
- Wire-parity corrections retained only where evidence supported them, without promoting live-share or live-result claims.
- Terminal no-promotion enforcement in Phase 30 and reopening guards for the archived Phase 28.1.1 lineage.

### Inserted Diagnostic Phases

Phase 28.1 isolated the firmware nonce-production blocker. Phase 28.1.1 and descendants 28.1.1.1–28.1.1.7 preserved successive diagnostic hypotheses, results, and exact non-claims. The full lineage closed as terminal unresolved work with every verification result remaining `gaps_found`.

### Accepted Unresolved Work
...
- **CFG-07:** Runtime-only credential handling exists, but no eligible live evidence root proved the requirement end to end.

Phase 30 therefore selected the conservative no-promotion disposition. These gaps may be reconsidered only through a newly planned milestone with explicitly supplied new evidence; the archived diagnostic lineage must not be resumed.

### Carried Process Debt

- Nyquist validation remains partial for Phase 28.1 and missing for Phase 28.1.1.1.
- Some Phase 23 and Phase 27 lifecycle metadata remains historical debt.
- The installed GSD `progress` path may not resolve active-milestone archives correctly. Do not repair that tool limitation by recreating active phase directories or changing failed verification results to passed.
