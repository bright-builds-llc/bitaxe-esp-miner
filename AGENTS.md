...
- Ignored local `pool-credentials*.env` files remain a fallback only when a command needs direct `BITAXE_POOL_*` environment variables. Use the committed `pool-credentials.env.example` file for fallback shape only; the real local file may define `BITAXE_POOL_URL=public-pool.io`, `BITAXE_POOL_PORT=3333`, `BITAXE_POOL_USER=<owner-btc-address>.bitaxe`, and `BITAXE_POOL_PASSWORD=x`.
