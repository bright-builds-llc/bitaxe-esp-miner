...
- If detection succeeds and an ignored local `pool-credentials*.env` file exists, agents may source it only as local runtime input for repo-owned live mining test or verification commands. The file may define `BITAXE_POOL_URL=public-pool.io`, `BITAXE_POOL_PORT=3333`, `BITAXE_POOL_USER=<owner-btc-address>.bitaxe`, and `BITAXE_POOL_PASSWORD=x`; treat the owner address, worker string, endpoint, port, and password as sensitive local test inputs. Do not print, summarize, commit, copy into evidence, or expose the file contents. Committed evidence may record category labels such as `pool_config: local-owner-supplied` only, never raw pool URLs, ports, users, workers, passwords, addresses, endpoints, tokens, or NVS secret values.
