...
- Before the first substantive plan, review, implementation, diagnosis, audit, or answer about repository work, resolve and load the global active lesson input named by the loaded global instructions plus exactly one repository input: `tasks/lessons.md` when the repository root contains `tasks/`, otherwise `.codex/tasks/lessons.md`. Resolve both paths canonically and de-duplicate them when they identify the same file. Missing active files count as zero bytes. Exclude every archive file and archive directory from startup loading.
...
- When a repo-owned test or verification command needs `DEVICE_URL`, derive it from fresh monitor output after same-session detection. The only fallback is the task-gated `just native-usb-display-recovery` Interface described in `docs/hardware/native-usb-ownership.md`, which requires a board-displayed RFC1918 address and exact USB/API MAC binding. Neither path authorizes mDNS, ARP, router state, scans, stale logs, or unrelated evidence. Local development may display IPs; committed evidence must redact raw URLs, IPs, endpoints, MACs, Wi-Fi values, credentials, workers, passwords, tokens, and NVS secret values.
...
- Phase 28.1.1 and its seven numbered child phases are terminal archived unresolved history under `.planning/milestones/v1.1-phases/`.
...
- Replay intended to prove a cold boot must be owned by a boot-lifetime task, not by Wi-Fi, HTTP, Stratum, pool, or ASIC-session progress. Transport readiness and application-service readiness are separate evidence boundaries; a clean passive attachment with zero bytes must retain both facts rather than treating attachment as proof that replay ran.
- An always-on firmware heartbeat proves boot age only after the serial transport delivers application bytes. Native-USB node appearance, stable identity, expected ownership, heartbeat production, and observed bytes are independent facts; do not infer byte delivery from the first four or retry a zero-byte cold capture without a newly planned transport-level diagnostic.
