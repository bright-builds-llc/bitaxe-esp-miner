...
type PreparedDiagnostic = {
  ...
  readonly wifiPath: string; readonly poolPath: string;
  readonly backup: JsonObject; readonly restoreBundle: RestoreBundle;
  readonly restoreBundlePath: string; readonly host: string;
  readonly expectedPeer: string;
};

export class TcpPayloadDiagnosticError extends Error {
  public constructor(
    public readonly category: string,
    public readonly checkpoint: string,
  ) {
    super(`${category}:${checkpoint}`);
    this.name = "TcpPayloadDiagnosticError";
  }
}
...
export function tcpPayloadDiagnosticWorkspaceRoot(
...
export function parseTcpPayloadDiagnosticArgs(
  ...
  const allowed = new Set([
    ...
    "--private-parent", "--projection", "--plan", "--diagnostic-ordinal",
    "--capture-timeout-seconds", "--redact-evidence",
  ]);
...
async function preflight(workspace: string, args: TcpPayloadDiagnosticArgs): Promise<PreparedDiagnostic> {
...
async function writePrivateText(candidate: string, value: string): Promise<void> {
...
export async function inspectTcpPayloadDiagnosticPreflight(
  ...
  return {
    schema_version: "bitaxe-stratum-v2-tcp-payload-preflight-v1",
    status: "ready",
...
export async function runTcpPayloadDiagnostic(
  workspace: string,
  args: TcpPayloadDiagnosticArgs,
): Promise<JsonObject> {
  ...
  try {
    ...
    diagnosticChild = await runManagedDiagnosticProcess(
      ...
      [
        "tcp-payload-diagnostic", "--board", "205", "--port", args.port,
  ...
  await writePrivate(path.join(privateRoot, "fixture-child.private.json"), {
    ...
    stdout_sha256: sha256(Buffer.concat(fixture.stdout)),
    stderr_sha256: sha256(Buffer.concat(fixture.stderr)),
  });
  await writePrivateText(
    path.join(privateRoot, "fixture.stdout.private.log"),
    Buffer.concat(fixture.stdout).toString("utf8"),
  );
  await writePrivateText(
    path.join(privateRoot, "fixture.stderr.private.log"),
    Buffer.concat(fixture.stderr).toString("utf8"),
  );
...
export function tcpPayloadDiagnosticFailureResult(error: unknown): JsonObject {
  return {
    schema_version: "bitaxe-stratum-v2-tcp-payload-result-v1",
    status: "failed",
    category: error instanceof TcpPayloadDiagnosticError
      || error instanceof ManagedDiagnosticProcessError
      ? error.category
      : "process_failed",
    checkpoint: error instanceof TcpPayloadDiagnosticError
      || error instanceof ManagedDiagnosticProcessError
      ? error.checkpoint
      : "unclassified",
