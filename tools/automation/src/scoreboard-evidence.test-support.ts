...
export async function scoreboardFixture(name: string): Promise<ScoreboardFixture> {
  ...
  const planRelative = "docs/parity/work-plans/20260818T064430Z-STAT-003/PLAN.md";
  ...
  const wrapper = path.join(root, "scratch/stat003-scoreboard/wrapper-001");
  ...
  return {
    ...
    options: {
      privateRoot: "scratch/stat003-scoreboard/attempt-001",
      packageManifest: "inputs/package.json",
      wifiCredentials: "inputs/wifi.json",
      poolCredentials: "inputs/pool.json",
      detectorOutput: "scratch/stat003-scoreboard/wrapper-001/detector.stdout",
...
if (args[0] === "mining-campaign") {
  ...
  const network = JSON.stringify({ schema: "mining-campaign-network-continuity-v11", status: "accepted", correlation_failure: "none", required_window_count: 20, covered_window_count: 20, work_renewal_valid: true, terminal_http_valid: true, terminal_websocket_valid: true, terminal_pool_persisted: true }) + "\\n";
