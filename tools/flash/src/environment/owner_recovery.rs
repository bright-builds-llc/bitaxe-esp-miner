use super::*;

pub(super) fn execute_rom_probe(
    environment: &LocalFlashEnvironment,
    command: &CommandSpec,
) -> Result<Vec<u8>> {
    if command.program != "espflash"
        || command.args.first().map(String::as_str) != Some("board-info")
        || command
            .args
            .windows(2)
            .filter(|pair| pair == &["--before", "no-reset"])
            .count()
            != 1
        || command
            .args
            .windows(2)
            .filter(|pair| pair == &["--after", "no-reset"])
            .count()
            != 1
    {
        bail!("owner_recovery=blocked reason=rom_probe_contract");
    }
    let mut session_slot = environment.usb_session.borrow_mut();
    let Some(session) = session_slot.as_mut() else {
        bail!("cleanup_failed: ROM probe attempted without a repository session");
    };
    let args = command_with_port(command, session.port())?;
    let output = session
        .run_espflash_probe(
            environment.espflash_bin.as_std_path(),
            &args,
            Duration::from_secs(30),
        )
        .map_err(|error| anyhow::anyhow!(error))?;
    let mut combined = output.stdout;
    combined.extend_from_slice(&output.stderr);
    Ok(combined)
}

pub(super) fn execute_exit(
    environment: &LocalFlashEnvironment,
    esptool: &Utf8Path,
    observation_seconds: u64,
) -> Result<OwnerRecoveryExitCapture> {
    validate_managed_esptool(&environment.workspace_dir, esptool)?;
    let mut session_slot = environment.usb_session.borrow_mut();
    let Some(session) = session_slot.as_mut() else {
        bail!("cleanup_failed: owner recovery attempted without a repository session");
    };
    let inspection = inspect_usb_profile(session.port())?;
    if inspection.physical_identity_digest != session.physical_identity_digest()
        || !matches!(
            inspection.profile,
            UsbProfile::SerialJtagRuntime | UsbProfile::RomDownloader
        )
    {
        bail!("physical_identity_drift");
    }
    let read = session
        .run_espflash_probe(
            esptool.as_std_path(),
            &force_download_read_args(session.port()),
            Duration::from_secs(30),
        )
        .map_err(|error| anyhow::anyhow!(error))?;
    let force_download_bit_set = parse_force_download_bit(&read.stdout)?;
    environment.validate_espflash_identity()?;
    let application = run_installed_application(
        session,
        esptool.as_std_path(),
        environment.espflash_bin.as_std_path(),
    )
    .map_err(|error| anyhow::anyhow!(error))?;
    let monitor = session
        .observe_receive_only(Duration::from_secs(observation_seconds.min(30)))
        .map_err(|error| anyhow::anyhow!(error))?;
    Ok(OwnerRecoveryExitCapture {
        force_download_bit_set,
        transport: application.transport,
        reenumerated: application.reenumerated || monitor.reenumerated,
        monitor,
    })
}

fn validate_managed_esptool(workspace: &Utf8Path, program: &Utf8Path) -> Result<()> {
    let canonical_workspace = fs::canonicalize(workspace.as_std_path())?;
    let canonical_program = fs::canonicalize(program.as_std_path())?;
    let relative = canonical_program
        .strip_prefix(&canonical_workspace)
        .context("managed esptool leaves workspace")?;
    let allowed = [
        ".embuild/espressif/python_env/idf5.5_py3.14_env/bin/esptool.py",
        ".embuild/espressif/python_env/idf5.5_py3.9_env/bin/esptool.py",
    ];
    if !allowed
        .iter()
        .any(|candidate| relative == std::path::Path::new(candidate))
        || fs::symlink_metadata(program.as_std_path())?
            .file_type()
            .is_symlink()
    {
        bail!("managed_esptool=blocked reason=program_contract");
    }
    Ok(())
}
