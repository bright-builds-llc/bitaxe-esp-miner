...
### task-fixed-usb-serial-migration | 2026-09-04 | Replace switching USB with fixed Serial/JTAG and Web Serial

- [x] Record successor decisions, contracts, guidance, and task dependency corrections in both repos.
- [x] Implement Gate Controller 0.4, serial 0.2, possession 0.2, signing/conformance and real browser/headless composition; remove obsolete active compatibility paths.
...
Transferred obligations: buttonless standard flashing, runtime stability, exact identity, settings preservation, cleanup and a qualified safe recovery baseline replace the mechanism requirements of `task-native-usb-ownership-handoff` and `task-native-usb-recovery-transition-205`. Their old failures remain failures. Recovery-006 is retired as this migration's mandatory destination, without claiming its missing application/settings/selected-byte evidence succeeded.

Verification: ordered Rust gates, Gate's own full browser/signing/crypto/build checks, Bazel, real ESP32-S3 package, fixed-USB ownership, redaction, reference/provenance, and diff review. No hardware until software gates pass and the preexisting stuck detector is cleaned up. Progress and residual risks remain below; no automatic parity promotion.

Progress: Gate requirements `8392d8a`, initial implementation `8f169a7d`, and final implementation `95af34a1d17f686ff7f511c22cef8597886c818e` are pushed; final archive SHA-256 `2b3b7a706faf830d86c79dbc09a72a572f910b62017f28a6b810c88bfa934393` is pinned. Gate full checks and 280 web/CLI tests plus browser conformance pass. Firmware host lint/build and flash tests pass; full integration, final package, and hardware evidence remain in progress. Canonical packaging detected a 46,480-byte slot overflow; application-only size optimization resolved it at 4,032,352 bytes while retaining the fixed partition layout. This is a software-check package, not the final clean hardware package.

Cleanup correction: macOS snapshot checks no longer open the serial driver; metadata/access and holder checks preserve physical ownership without repeating the blocked driver open. Actual ROM and runtime proofs remain explicit. The preexisting blocked process still requires cleanup before any new session.

Final-review correction verified in software: active time now runs from first dispatch through confirmed reset/disable, with a derived 15,550-ms pre-reset reserve. Unsupported/drifted plans and UART-trace timing are denied admission. Automated terminal reply/delivery bounds are 140/145 seconds; cooling remains 120 seconds. All 73 Bazel tests (including real ESP32-S3 packaging), ordered host Rust gates, ownership/symbols, reference and redaction checks pass. Physical ≤3-second initiation and active-window limits still require the qualification tasks.
...
Reconciliation | 2026-09-13: Implementation/publication, continuity, successor live acceptance and no-drain Hello recovery are supported by the exact-pair evidence in [the reconciliation](docs/parity/evidence/20260913-usb-qualification-reconciliation.md). Keep this parent active: `task-fixed-usb-serial-qualification` still lacks measured CPU0 main-telemetry cadence under hardware load. The original live campaign is superseded, not passed; no new campaign, hardware authority or parity promotion is created.

Cadence outcome | 2026-09-13: The new exact-pair preparation passed four fresh state-preserving cycles, but Gate rejected idle review before any USB-load or mining phase. Parent migration remains unresolved; see [cadence evidence](docs/parity/evidence/20260913-cpu0-telemetry-cadence.md). No earlier campaign result changes.

USB-capture follow-up | 2026-09-13: [Preparation 3](docs/parity/evidence/20260913-cpu0-cadence-usb-preparation.md) advanced through four cycles and initial idle review, but USB capture loss and timing prevented mining admission. Migration still depends on full cadence qualification after the verified recorder/log-path correction.

Current cadence dependency | 2026-09-14: [The recovery installation](docs/parity/evidence/20260914-cpu0-cadence-recovery-startup-blocker.md) reached healthy runtime but remains unqualified because of its initial panic reset category. Its single installation claim is consumed; no browser or mining started. Fresh stable recovery/accounting and complete current-pair cadence evidence remain missing. Migration remains active without new effect authority or parity promotion.
...
- [x] Verify largest frames, fragmentation/coalescing, session replacement, foreground closure, heartbeat expiry and serial port ownership on macOS/Chrome.
...
Each attempt has a fresh mode-0700 parent, absent supervisor child, separate mode-0600 stdout/stderr, exact command/effect manifest, bounded ROM/transfer/recovery/cleanup operations, and preserved first failure. Human checkpoints have no deadline. No unchanged retry. Historical archive and consumed attempts are immutable. Only new evidence may support new qualification; old evidence cannot be relabeled for the new transport.

Qualification follow-up plan (2026-09-05): physical RESET of installed `49082929` established exact source/ELF identity, confirmed Worker no-mining baseline, and runtime-ready diagnostics; first startup failure is network. Browser receives those closed diagnostics but admission fails before possession, with no mining/cycle receipt. Preserve attempt-003 and its earlier missing automatic-return evidence.

- [x] Reproduce missing firmware peer heartbeats through the actual writer; implement advancing session-bound periodic output above diagnostics without extending host authority or leases.
- [x] Preserve Gate's admission stage across asynchronous closure and expose only closed failure categories.
...
Software progress: the actual writer regression failed without peer heartbeats and passes with periodic advancing output, including continuous control traffic. Browser regressions reproduced missing asynchronous admission stages and falsely retained ownership after successful port closure; fixes preserve the first closed error and track actual late native settlement. Entropy/network host tests and ESP32-S3 compilation pass. A typed first network phase/error is replayed without arbitrary exception text. Full coordinated verification and hardware requalification remain pending.

Peer/reset verification: Gate `16a86e4443c57dd999984fac364193c3fe3d285d` is published and pinned with archive SHA-256 `43d9eed0bfce5f75e852116a1982e1caa510fa401e7d5790bed483b8f595ee0c`; full Gate verification passes, including 295 web/CLI tests and browser conformance. Firmware ordered Cargo gates pass (2,045 tests), and the final reduced-concurrency Bazel run passes all 76 targets. Earlier child-startup/fixture failures remain recorded; no deadline or assertion was relaxed. USB ownership/symbols, reference, redaction, standards and diff checks pass. Stored Device Identity is preserved; this code correction makes no retrospective claim about its original entropy. Hardware automatic return, browser possession, maximum frames and all cycle/mining acceptance remain unverified.

Attempt-004 correction and next plan (2026-09-05): the flash command's rejected trust summary did not mean absent application output. The protected 22,892-byte capture contains exact `16c0c1fe`/ELF identity, one observed transition from boot 2 to boot 3 with panic reset reason, then stable runtime-ready output. The retained allocation receipt is not proof of that panic's cause. Automatic return reached the application; startup stability remains unqualified. The browser subsequently verified possession and the no-mining baseline with settings/authorization/identity comparisons true. Its maximum-size exchange then failed with `liveness_lost`; resources were released. No cycles or mining completed.

- [x] Construct Wi-Fi after safe hardware setup but before BWG/HTTP allocations, keeping blocking connection startup deferred and all reserves/stack sizes unchanged. Measure construction boundaries and require every later owner to initialize.
- [x] Reproduce large-record deadline/flush behavior using the actual transport seam; preserve 2.8-second revocation while correcting bounded write/flush timing and heartbeat scheduling as justified.
- [x] Record only closed transport failure/timing observations, keep the failed probe immutable, and rerun affected cross-language/native checks before coordinated publication and a fresh hardware attempt.
- [x] Make current package flash/monitor assessment consume fixed-Serial/JTAG identity, baseline and startup records, distinguish rejected startup from missing execution, and retain exact identity/stability/privacy checks instead of requiring obsolete plaintext boot output.

Maximum-exchange software evidence: the production USB write-loop seam reproduced rejection of a transfer whose write plus 40-ms drain fit the existing two-second budget. The corrected shared write/drain budget passes, including rejection of a drain beyond that budget. Actual-writer tests verify a fresh peer heartbeat before long control replies. Controller/Gate tests now exercise exactly 65,536-byte requests and responses; oversize/truncating response requests are rejected. Closed TX failure diagnostics contain stage, elapsed time and queued/record byte counts only; queued bytes do not prove delivery. Hardware resolution remains pending.

Maximum-exchange consumer pin: Gate `676ed256f368b6910fbbc299353ae8490f9aa778` is published, with archive SHA-256 `9a0df99461d5dfffd5cd15b9c273581a642f088c11720958621d975f0581086b`. Full Gate verification passes (296 web/CLI tests plus Rust/type/build/browser/package/standards). Current flash/monitor reports now assess fixed Serial/JTAG execution, safe baseline, startup and stability directly; classic transcript/attestation fields are not applicable to that path. Tests preserve history while rejecting current identity conflicts, reboots and startup failures. Final paired firmware checks and hardware resolution remain pending.

Maximum-exchange final verification: the paired Gate pin passes full verification; firmware ordered Cargo gates pass (2,065 tests, one preexisting ignored), and all 77 Bazel targets pass after pinning, including native ESP32-S3 packaging and the fixed-serial assessor. Required boot-validation false/error results now remain visible as startup failure. Wi-Fi/BWG/HTTP construction ordering, bounded write/drain behavior, exact bidirectional payloads, privacy and capture/tamper cases are covered. Hardware memory improvement, maximum exchange success, startup stability, 20 cycles and bounded live mining remain pending.

Attempt-005 evidence and re-plan (2026-09-05): `91bca0f6` flashed with five verified segments and returned automatically to exact execution. Capture assessment confirms a stable boot and safe baseline but rejects current `storage_http` startup failure. Wi-Fi construction succeeds (238567→161639 internal/DMA bytes); later required-service health is not established. Browser possession/baseline succeeded, but the maximum exchange still ended in liveness loss. No cycles or mining ran.

- [x] Add closed first SPIFFS/HTTP phase/error and service outcome evidence before selecting any resource fix; no speculative stack/reserve/threshold change.
- [x] Preserve failed maximum probe and inspect retained TX evidence: zero TX failure records, stable boot 4, browser/supervisor/monitor closed and no port holder. Add first closed RX stage/count evidence without changing buffers or deadlines.
- [x] Have the probe report/verify the actual received request padding length so response extension cannot mask dropped fixed-pattern input; cross-language exact-bound regression fails before and passes after correction.
- [x] Exclude valid clean-close frames from RX failures, clear receipts before publishing a fresh epoch, and clear revoked partial input before reconnection; preserve exact session/counter validation.
- [x] Verify and publish coordinated diagnostic/protocol changes before a new bounded hardware attempt; keep all earlier failures and the one campaign budget intact.

Diagnostic software verification: ordered Cargo format/lint/build/test gates pass (2,070 tests, one preexisting ignored). Actual-writer and storage/error-classification host targets pass; ESP32-S3 compilation, native USB ownership/symbols, reference integrity, semantic redaction and standards checks pass. Review confirmed strict clean-close handling, receipt clearing before fresh-epoch publication, revoked partial-input cleanup and exact probe byte counts. The unused PHY-handoff source alias was removed with no active caller. Gate `61cf6c599807abbdb12d88f5b810df73e564c757` is published and pinned with archive SHA-256 `9ae921f00ad46f0919c90689e07ef80ede65cd3e1e6af4531dd9ff6feefff0ff`; its full verification passes, including 299 web/CLI tests and browser conformance. Final pinned Bazel verification passes all 78 tests, including native packaging. This revision is ready for the exact clean package and a fresh bounded no-mining diagnostic attempt; hardware startup health, maximum exchange success, all 20 cycles and live acceptance remain unverified.

Attempt-006 evidence and next plan (2026-09-05): clean `0bdcbe29` installed all five verified segments and returned automatically to stable exact execution. The new first failure is `http_telemetry_worker/no_memory`; SPIFFS is available but HTTP startup did not complete. No browser or mining session was opened. The command exited, no owned process remains, and both serial nodes have no holder. The same campaign remains unused.

- [x] Qualify the documented ESP-IDF memory-saving Wi-Fi buffer profile (static RX/TX 6/6, receive window 12, dynamic RX 32) for the measured allocation failure, accounting for retained HTTP and later USB/statistics owners. Preserve all stacks, the internal reserve, safety owners and scheduling.
- [x] Reject stale/missing/duplicated resolved buffer settings before build artifact publication, restore the omitted build-validator test to the canonical suite, remove its obsolete TinyUSB fixture, and repeat required checks and clean packaging.
- [x] Require healthy startup before the next browser probe; retain RX diagnostics for any subsequent transport failure.
...
- [x] Commit/push the four-cycle policy and both repositories' operational/specification corrections; preserve closed records and original attempt context.
- [x] Seal an amendment binding the original context, four report digests, protected original package/browser artifacts and separate qualification-source revisions. Permit only reviewed qualification/documentation changes, with original firmware `11436c7e` and Gate `61cf6c5` runtime identities exact.
- [x] Restart the supervisor on the same origin, retain the same browser page/baseline, and establish fresh possession before signed mining. Do not reflash or discard the valid four cycles for this policy-only change.
...
Four-cycle amendment completion (2026-09-05): firmware policy revision `b935ebbe` and Gate policy revision `369b0b9f` are pushed and clean. Gate full verification passed after an unchanged-bound browser-fixture bootstrap retry; the original bootstrap failure and cleanup remain recorded. The sealed amendment binds the original attempt-008 context, four reports, and thirteen protected artifact copies. Fresh permission/possession and an actual 65536-byte request/response exchange passed with the original browser preservation baseline. Actual native tab hiding and return were observed in a no-mining dry-run, with temporary observation code and the temporary tab removed. Original runtime identities remain firmware `11436c7e` and Gate `61cf6c5`.

Attempt-009 receive-loss re-plan (2026-09-05): firmware `eb67af45` / ELF `ec1d924d` and Gate `368adbd8` passed required software checks and were published. The initial install and all four subsequent flashes returned to healthy exact no-mining execution with cleanup. Cycle receipts 1–3 completed; cycle 3 retained an earlier failed probe and a later successful fresh session. Cycle 4 has no successful receipt: its maximum probe failed even after displayed readiness, disproving sequencing alone as a sufficient explanation. A temporary, source-hash-matched, non-pausing diagnostic observed pass/pass/fail within one fresh session: expected request padding 65376, actual 63584, response padding 65396 and all `x`. This proves 1792 missing input bytes, not an uninstrumented timing result. The driver overflow mechanism is supported by exact source inspection, not an observed overflow counter. All debugger observations were removed. Fresh recovery confirmed restoration, inactive lease, unchanged preservation and mine-on-boot false; browser and supervisor exited and all holders were absent. No mining window was issued in attempt-009. Thirteen original runtime artifacts were copied and verified before further source changes.

- [x] Implement successor ADR-0023 / Gate ADR 0096: serial 0.2 receive credits and exact payload length/digest before dispatch, with unchanged 64/65 KiB and safety bounds.
...
Reconciliation | 2026-09-13: The [obligation matrix](docs/parity/evidence/20260913-usb-qualification-reconciliation.md) resolves publication, configuration/build guards, combined memory-profile/main-handoff startup, largest frames, session replacement, foreground/heartbeat closure and resource ownership using their actual software and hardware evidence. Fragmentation is observed in retained browser reads; arbitrary fragmentation/coalescing coverage is deterministic software evidence, not an exhaustive physical chunk-boundary claim. Original attempt failures and historical progress statements remain unchanged.

Blocker recorded 2026-09-13: [Preparation 3](docs/parity/evidence/20260913-cpu0-cadence-usb-preparation.md) passed four continuity cycles and its initial idle review, then failed USB capture integrity and the interval percentile (85/91 within750ms, nonzero loss indicator and two pending send observations). Mining was never issued. Recorder outcome accounting and unused retained-log copying are corrected in software, with exact-seal-bound v2 continuation implemented. A new clean published pair must repeat four cycles and all three captures, independent heartbeat shutdown, cooling and cleanup under unchanged limits. No failed capture is promoted and no ledger reservation is refunded. Keep qualification and migration active until that full evidence passes.

Environment blocker recorded 2026-09-13: The [clean package build](docs/parity/evidence/20260913-cpu0-cadence-host-build-blocker.md) is blocked with the host dependency child observed at `_dyld_start` and no Rust frames; Rust entry was not observed. Verified corrections are published, but preparation 4 has no context or hardware authority until clean packaging succeeds.

Resumed qualification | 2026-09-14: [The host investigation](docs/parity/evidence/20260914-host-launch-investigation.md) establishes the policy-wait cause and successful unchanged launch/package recovery. [Funded ordinal 16](docs/parity/evidence/20260914-cpu0-cadence-mining-016.md) subsequently passed four cycles, idle, USB and independent shutdown, but mining missed the unchanged percentile requirement (90/96 within 750 ms). Its charged result and cleanup are sealed. Qualification remains active for a verified targeted correction and a complete fresh exact-pair run; no prior campaign is promoted.

Current blocker | 2026-09-14: [The corrected recovery pair](docs/parity/evidence/20260914-cpu0-cadence-recovery-startup-blocker.md) has healthy observed startup and advancing timestamps in one captured boot, but its initial panic reset category fails the unchanged startup assessor. The preceding reset's timing and firmware attribution are unknown. Preserve the failed receipt and consumed installation claim; no recovered result or fresh qualification preparation is admitted. Next bounded work is prospective reset-origin diagnostics/read-only observation, then fresh authenticated baseline/accounting before complete cadence qualification. Keep this task active.
...
Migration dependency: `task-fixed-usb-serial-qualification` remains open: the [latest recovery attempt](docs/parity/evidence/20260914-cpu0-cadence-recovery-startup-blocker.md) reached healthy runtime but failed panic-origin startup admission. Fresh recovery/accounting and complete current-pair cadence qualification remain unverified. [The reconciliation](docs/parity/evidence/20260913-usb-qualification-reconciliation.md) retains the separately satisfied continuity, live-stop and Hello-recovery obligations. Qualification replaces TinyUSB handoff/recovery-006 requirements. The immutable legacy plan is historical and requires a successor contract before effects. Independent authorization, replay, monotonicity, protocol and evidence blockers remain unresolved.
...
Stage-B guarded restart | 2026-09-14: Adopt [the prospective restart contract](docs/hardware/reset-origin-restart-qualification.md) under the existing bounded hardware authorization. Accepted Stage A is retained below. One new clean published pair, one state-preserving installation and one authenticated restart are eligible only after implementation, software/native verification, source-bound preflight and exclusive claims. No mining or cadence continuation is conveyed by this stage.

- [x] Implement source-bound restart preflight, exclusive assignment/install/restart claims and factual installation review.
- [x] Implement one new-Gate private baseline across old/new firmware, independent new-pair observation, bounded authenticated restart and unchanged accounting.
- [x] Verify scoped synthetic filesystem/HTTP/two-client behavior and actual Gate configuration parsing; correct pending replacement-possession admission and post-ACK old-boot readiness handling.
- [x] Correct temporary executable test-fixture launch through explicitly selected interpreters while retaining real process boundaries and timeout limits; verify against the captured macOS pre-entry policy-wait recurrence.
- [x] Implement and verify `cadence-preflight --supersede-restart <accepted-Stage-B/result.json>` before freezing the pair: exact runtime/charged lineage, preparation 17-2, exclusive sibling assignment, unchanged four-cycle/three-phase criteria and fresh device accounting. The command grants no continuation without accepted Stage-B evidence.
- [ ] Complete canonical/native verification, publish the exact pair and build its clean package.
- [ ] Execute one guarded installation, new-pair observation and authenticated restart; independently review and seal the evidence with actual cleanup.
- [ ] Admit a fresh cadence successor only after accepted controlled-restart evidence; preserve all original outcomes and charged accounting.

Stage-B review corrections: firmware rejects pending replacement possession proofs; Gate retains post-ACK old-boot diagnostics without crediting them as new-boot readiness; the host supervisor keeps terminal failure sticky through asynchronous verification and final permit response. All three boundaries have regression coverage. [The host fixture investigation](docs/parity/evidence/20260914-automation-fixture-launch.md) records the separate macOS pre-entry policy-wait recurrence and explicit-interpreter correction; all 507 canonical automation tests pass without changing production launch behavior or timeouts.

Stage-B publication verification | 2026-09-14: Ordered Cargo format/Clippy/build/tests passed (2239 tests, one existing ignored). Gate `e3f4ce6283ebeb4f58479aa66e530f9ea7bcbfd0` is clean/published with all 592 WebCrypto tests, browser/type/package checks and ordered Rust checks passing. The integrated firmware/host changes pass all 127 canonical targets, including the actual Gate parser, both browser clients through real HTTP and independent judgment, sticky-failure races and conditional cadence admission. Native preview build and USB symbols pass; owner entry is 4512 bytes within its 8192-byte audit bound, and the known telemetry call path is 12560/16384 bytes (not a complete callgraph bound). Ownership, reference, redaction, standards, task IDs and parity/progress checks pass. Simplification review retains the existing process adapter and historical judges, using narrow test interpreter mappings and a separate verified-evidence lineage validator. Publish this implementation and build the clean exact package before source-bound preflight. No Stage-B installation/restart or new mining reservation has occurred; hardware proof remains open.
...
- [x] Run ordered software, cross-language, native storage/stack/package and safety/ownership checks; publish the exact Gate/firmware/observer pair before effects.
...
Preparation-002 review | 2026-09-13: Exact firmware 1d60fb1e / Gate 6870218 passed initial installation and four fresh state-preserving update/reconnect cycles, each with healthy startup and 65536-byte bidirectional exchange. The passive observer connected and idle was armed. Idle review failed at the controller's pre-possession `probe_admission` guard; fresh diagnostic recovery failed at the post-possession guard. No idle summary was collected, USB/mining did not start and no allowance was issued or reserved. Fresh authenticated ledger remains 1200000 ms / next 16 / last 15 / no pending. Final safe baseline, inactive lease, browser/USB release, supervisor exit 0 and absent owned children/listener/holders are observed. See [the preserved failure report](docs/parity/evidence/20260913-cpu0-telemetry-cadence.md).

Correction review | 2026-09-13: Gate ad16c23d is published and pinned after all 352 Rust tests (two existing ignored), 549 web tests, browser conformance, types, packaging and standards passed. Five actual-controller regressions reproduce both normal heartbeat-write races and retain bounded timeout/cancelled-partial recovery. Only the cadence guard's generic unfinished-record rejection was removed; the existing bounded send queue, concurrent-control/work guards and device authority deadlines remain unchanged. Hardware evidence still belongs to Gate 6870218. The failure-only inventory verifies 102 original files / 42 journal rows; it grants no continuation authority.

Remaining work: Retain this preparation as failed/unverified. The existing permission-only unissued classifier is inapplicable after idle/observer activity; the funded judge also must reject unissued work. Add a distinct pre-mining failure closure and guarded supersession before a fresh complete exact-pair qualification. No relaxation, ledger refund, old-context reuse or parity transition is authorized by failure cleanup. Cadence, USB qualification and migration remain active; no completion/archive review is claimed.

Final outcome verification | 2026-09-13: Corrective pin passes ordered root Cargo gates (2209 passed, one existing ignored), all 272 supervisor tests, all 109 canonical targets, native package, USB ownership/symbols, reference, redaction and standards checks. Failure inventory revalidation and independent evidence/privacy review pass; archive/checklist bytes are unchanged and 230 task IDs remain unique. Parity stays 90/95 active verified. This is a verified software correction and truthful failed preparation, not completed hardware qualification.

Guarded pre-mining continuation | 2026-09-13: The owner explicitly approved implementing a distinct effect-free closure and guarded supersession for preparation 2, followed by a complete fresh qualification with corrected Gate ad16c23d and a new clean published firmware package. Preserve both sealed preparations and all ordinal markers. Closure is an exclusive protected sibling file, never a write inside the sealed root, and grants no effect authority. Only verified software correction, unchanged charged predecessor/unused ordinal, fresh preparation/allowance identity, exact published artifacts, active task and fresh authenticated accounting admit preparation 3. Permission-only closure and funded judges retain their original guards.

- [x] Implement and test independent pre-mining evidence verification, sibling closure/review commands and mutually exclusive supersession admission, including immutable history and partial-assignment rejection.
- [ ] Verify Gate regressions and ordered root/native/canonical/safety/privacy gates, publish the source pair and build the clean exact package before effects.
- [x] Close and review preparation 2 without changing its sealed bytes; admit preparation 3 with expected unused ordinal 16/charged 1200000 ms and a fresh allowance identity.
- [ ] Perform initial state-preserving installation and four fresh exact-pair continuity cycles; collect idle, USB-load and mining cadence under unchanged criteria, then independently prove heartbeat shutdown, cooling and final cleanup.
- [ ] Seal and review the exact hardware outcome; archive cadence/qualification/migration only on a complete pass, otherwise retain precise gaps; update dependencies, commit and push without parity promotion.

Effects remain the existing complete cadence contract: one detector-admitted Ultra 205, physical lease/ROM proof before disjoint updates, native trusted browser gesture, one fresh authenticated passive subscriber, three 60-second captures, twelve serialized 65536-byte probes five seconds apart, and one 180000-ms normal reservation for 60 seconds actual mining. Every consumed reservation is fully charged; expected completion is next 17 / last 16 / 1380000 ms / pending false. No old-context reuse, allowance refund, threshold relaxation or extra discovery. On a recurring same authoritative failure signature after its verified fix, stop and retain evidence per the hardware-attempt policy; distinct findings require targeted regression-backed correction before continuation.

Pre-mining closure software review | 2026-09-13: Independent review added the already-published failed-inventory hash as an immutable initial anchor, so coherent evidence-plus-inventory replacement is rejected. New supersession reserves its exclusive preparation marker before child/copy operations; interruption leaves it unavailable for reassignment. Canonical real review caught relative process paths being resolved from the Bazel launch directory; resolution now uses the context-bound firmware root with same-root containment, and a foreign-directory dispatcher/path-escape regression passes. Neither sealed preparation was changed; no real pre-mining sibling closure was created during these checks.

Verification before publication: Ordered Cargo gates passed (2209 tests, one existing ignored), full supervisor suite passed 284 tests, all 110 canonical targets passed, and the final 14-test affected suite plus canonical target passed after the path correction. Native package, ownership/symbols, reference, redaction, standards and scoped Markdown checks pass. Gate ad16c23d remains clean/published with 52 focused and 352 Rust/549 web tests passing. Canonical read-only review accepts the original 102-file/four-cycle/five-flash preparation and exact 6895f2fc seal. Archive and earlier report/checklist bytes are unchanged; 230 task IDs remain unique and parity is 90/95. Next: publish, build the clean exact package, create/review the sibling closure and admit preparation 3 before hardware.

Preparation-003 outcome and targeted continuation | 2026-09-13: Published 912e7a98 / Gate ad16c23d passed initial installation and four fresh exact-pair cycles. Idle initially passed 95 intervals. USB completed all 12 probes but failed capture loss and the 95% criterion (85/91 within 750 ms; max 876.213 ms, processing 369.667 ms). The loss indicator is saturated 1, with two pending send observations persisting after fresh authenticated review. No mining allowance was issued, armed or consumed. Fresh ledger remains 1200000 ms / next 16 / last 15 / pending false; safe baseline and all browser/USB/observer/supervisor cleanup were verified. [The failure report](docs/parity/evidence/20260913-cpu0-cadence-usb-preparation.md) binds the immutable 107-file seal 1936ad2be4fffa59dbc650fd833bc335e280dd31394ab3378e5d2760aa1ab580. Preserve the original passing idle receipt and later global-review flag change; no hardware success is inferred.

The same active contract authorizes targeted corrections and a distinct verified-progress continuation: record asynchronous outcomes independently of timing-store contention; retain prior phase results while campaign loss still fails; skip retained-log cloning when no log subscriber exists. Keep every cadence/storage/safety limit and 500-ms sleep/CPU0/priority 5/stacks unchanged. Add a separate exact-hash-bound v2 pre-mining failure class for this sealed USB failure, leaving v1's idle-review matcher and 6895f2fc anchor unchanged. Version dispatch may share close/review commands, but v2 independently verifies successful original idle evidence, 12 probes, both failed USB reviews, no issuance/work, unchanged ledger and cleanup. Write only a protected sibling; no closure grants authority. Existing guarded supersession may allocate preparation 4/new allowance ID for still-unused ordinal 16 only after verified fixes, clean published pair, original charged predecessor and exclusive assignment. Then repeat all four continuity cycles and three captures; no old phase/cycle transfer or threshold relaxation. Stop a repeated authoritative signature after its targeted fix per policy.

USB-failure correction verification | 2026-09-13: Send outcomes now use three bounded atomic banks independently of timing-store ownership; phase-local loss preserves prior phase results while campaign loss still rejects qualification. Seven recorder regressions pass, including actual concurrent callbacks with the timing guard held, queue/send failure, overflow and phase isolation. The periodic log path skips retained-buffer cloning without log subscribers; four real-helper/state regressions preserve membership-lock release, reconnect cursors and poison reporting. Ordered Cargo gates pass (2216 tests, one existing ignored); 297 supervisor tests and 112 canonical targets pass. Native package and ownership/reference/redaction/standards/parity checks pass. Native cadence storage is 588 bytes; owner entry remains 4496 bytes with unchanged 24576-byte stack. Independent recorder and v2 evidence reviews found no blockers; canonical real v1/v2 reviews preserve both exact failure seals. No timing threshold was relaxed. Publish this correction before the exclusive v2 sibling closure, preparation 4 admission, clean package and fresh hardware qualification. Parity remains 90/95.

Host continuation blocker | 2026-09-13: Corrections 4c537470 and Gate ad16c23d are published; all source/software/native-check gates passed before the clean final package. That clean build stalled while the esp-idf-svc host build-script child was observed at `_dyld_start` with no Rust frames; Rust entry was not observed, and Cargo was waiting for child output. Static signature verification passed; AMFI launch messages are diagnostic context, not a proven root cause. The exact owned child was terminated, package failure retained, and its Cargo/wrapper/child cleanup verified; unrelated project processes were untouched. Computer Use denied Terminal access for safety, so the alternate launch comparison could not run. The canonical closure launcher then stalled in workspace status and its identified client was interrupted. Direct invocation of the same published effect-free Node backend closed/reviewed preparation 3 through unchanged production guards; v2 sibling created, original 1936ad2b seal unchanged. Preparation 4's progress input exists, but its context and assignment marker were never created. No new hardware or reservation occurred. See [host blocker and resume requirements](docs/parity/evidence/20260913-cpu0-cadence-host-build-blocker.md). Require a clean exact published package after host recovery, fresh accounting and full qualification; all three tasks remain active.

Final blocker-record verification | 2026-09-13: Ordered host Cargo checks reran successfully through the repo host-stall recorder (2216 tests, one existing ignored; all four owned process groups cleaned). The existing built semantic redaction verifier passed with normal launcher environment without invoking the stalled Bazel workspace-status step; no verifier condition was bypassed. Standards/Markdown/diff checks pass. No workspace-status process remained after cancellation. This verifies the truthful blocker record; clean native packaging and full hardware qualification remain unresolved.

Host investigation and resumed ordinal-16 result | 2026-09-14: Exact-path/PID/thread/reference host logs identify an AppleSystemPolicy execution-decision wait. Cancellation interrupted it; the service response arrived about 601 seconds after launch and found the process dead. Current direct launches and a clean 73ad71c6 package pass without reboot, source/signature/security changes; why assessment was delayed remains unknown. The resumed preparation 4 passed initial installation and four actual cycles (a cycle 3 observer-path setup failure was corrected before any flash, with both observations preserved), idle and USB. Mining ran 73,081ms with 35dispatches / 3 renewals, but 90/96 intervals met 750ms (93.75%). Zero loss/pending/publication errors; revocation 2805ms/shutdown 2850ms and cooling/cleanup passed. [The funded failure report](docs/parity/evidence/20260914-cpu0-cadence-mining-016.md) binds canonical result 431ffc03 and immutable 127-file inventory a379f26c. Full 180000ms charged; next 17 / last 16 / 1380000 ms / pending false; original 240000ms ledger unchanged.

Next verified correction remains under this active task: retain a positive-only actual mounted asset-version fact for the boot (never cache initial missing/malformed results; future same-boot WWW replacement must invalidate); add bounded 11-stage live timing and one worst-interval witness joined to the previous iteration. Diagnostic schema v2 is additive to historical v1 readers; new preflights require v2 evidence before phase advancement/signing. Preserve 500ms sleep, CPU0/priority 5, stacks, <=2KiB static recording storage, no allocation/logging/durable writes/waiting locks in recording, all current percentile/publication/safety criteria and real mining authority bounds. After full tests/exact publication, ordinary preflight takes preparation 4/result.json for ordinal 17, without supersession/refund, then repeats four cycles and all three captures. No old result or first-cause artifact may be rewritten.

V2 software verification | 2026-09-14: Gate 82992de9 is published with 352 Rust tests (two existing ignored), 567 web tests and full checks passing. Root ordered Cargo gates pass (2225 tests, one existing ignored), all 304 supervisor tests and all 113 canonical targets pass. Independent host/cache reviews found no blockers. Native verification caught and corrected the Worker-control crate missing from build-provenance dependencies; recorder-only edits now invalidate the native build. Shared non-generic profiling bookkeeping removed duplicated code without changing measurements. Both images fit the unchanged 4-MiB OTA partition (normal 4190288 bytes, rollback 4189136; normal headroom 4016 bytes before final clean publication). Ownership, reference, redaction, standards and Markdown checks pass; parity remains 90/95. Tight image headroom remains a packaging constraint, never permission to enlarge partitions or omit diagnostics.

Optimized native audit: recording symbols total 1284 bytes; production-owner entry frame 4512 bytes remains below its 8192-byte entry budget with the unchanged 24576-byte task stack. Main entry is 64 bytes, telemetry loop 48, live adapter 208, iteration 224, recorder iteration 384 and diagnostic snapshot 2032. These are entry-frame observations, not a complete call-chain bound; fresh hardware owner-free-stack evidence remains mandatory. Exact clean final packaging must reconfirm image headroom before effects.

Ordinal-17 startup failure | 2026-09-14: Clean published a261a2d5 / Gate 82992de9 passed package admission (4190208-byte OTA image), exclusive context assignment and thirteen-artifact snapshot. Detector admitted one Ultra 205. Initial state-preserving write completed, but the 30-second observation failed startup qualification: first statistics-stage failure, repeated panic resets and non-monotonic uptime. No browser was opened, no cycle or cadence phase ran, and no allowance was issued. An 8192-byte internal-memory allocation receipt is retained chronology, not proof of the panic cause. Native comparison found additional large snapshot forwarding frames; exact call-chain diagnosis is in progress. Do not reuse this preparation or claim fresh accounting/restoration from the previous result.

- [x] Verify the new native stack-materialization boundary, remove accidental forwarding copies without changing task stacks or snapshot semantics, and rerun software/native checks with emitted caller/callee evidence.
- [x] Preserve and independently seal the initial-startup failure with actual host cleanup; keep absent device restoration/accounting proof explicit. Existing permission-only and pre-mining closure classes remain unchanged.
- [x] Define and verify the narrow recovery-only continuation before effects: a new protected context binds the failed preparation, charged predecessor, verified correction and clean exact pair. Fresh stable identity and authenticated accounting precede qualification/mining admission. No threshold relaxation, fabricated pre-failure settings baseline, reservation refund or old-context restart.
- [ ] Execute the one guarded corrected installation and fresh recovery-only baseline/accounting review, seal actual cleanup, then admit preparation 2 and repeat all qualification cycles/phases.

Initial-startup failure review | 2026-09-14: [The preserved report](docs/parity/evidence/20260914-cpu0-cadence-startup-017.md) binds the failure-only inventory d65b506e, thirteen exact artifacts, original assignment and charged predecessor. Actual flash/observer exits were 1/0; the unused supervisor exited 0 and all owned listeners/processes/USB holders are absent. No browser or signing activity occurred. Device restoration and fresh ledger review remain unverified. Exact failed-ELF disassembly proves a 16464-byte telemetry call path against the unchanged 16384-byte main stack; large diagnostic forwarding temporaries are the targeted correction. The allocation receipt remains non-causal chronology. A separate recovery-only context must restore stable evidence before a fresh qualification preparation; no old closure classifier is broadened.

Startup correction and admission verification | 2026-09-14: Direct start/finish timing avoids generic large-return temporaries, and ordinary HTTP/BAP/statistics callers retain unprofiled implementations on the same publication owner/lock. A mixed-path chronology regression passes. Ordered Cargo gates pass (2226 tests, one existing ignored), all 116 canonical targets pass, and 16 focused startup-recovery/supersession tests pass. The native targeted main audit rejects the frozen image at 16464 bytes and passes the corrected emitted path at 12560; owner entry remains 4512 bytes. Additional emitted-path comparisons found no remaining introduced ordinary-caller regression; they are not full callgraph or hardware-safety claims. Ownership, reference, redaction, standards, Markdown and parity/progress checks pass. Recovery is signer-free, requires an exclusive installation claim inside the prearmed process observation, verifies fresh accounting/close, and preserves both interrupted assignment markers. Frequent metadata recording avoids repeated artifact rehashing while admission/accounting retain frozen-source verification. Publish before clean-package recheck and the single recovery installation; parity remains 90/95.

Recovery-1 outcome | 2026-09-14: Published daa69bb0 / Gate 82992de9 passed clean packaging and native audits. The one guarded state-preserving write completed, but the unchanged assessor returned `reboot_observed` and `insufficient_advancing_samples`. Captured startup is complete with no startup failure; eleven boot markers remain in ordinal 2 with ten advances, and forty-seven startup-complete markers advance forty-six times. The periodic panic reason is boot-time history, not repeated panic events; its origin/timing remain unknown. [The blocker report](docs/parity/evidence/20260914-cpu0-cadence-recovery-startup-blocker.md) preserves this distinction. No browser/supervisor, accounting review, signing or mining started. Flash/observer exits are 1/0 with actual process/USB release. The installation claim remains consumed and no recovered receipt or preparation-2 assignment exists. Disposition: `stop_impossible_contract` for this immutable failed installation, not permanent impossibility. Preserve thresholds and all failed evidence; next work requires a separate prospective reset-origin observation contract. Cadence, qualification and migration remain active; parity stays 90/95.

Final blocker review: The independent failure inventory 0ed37434 seals thirty original files and thirteen artifacts; the prior d65b506e seal is unchanged. Read-only live-context checks reject both sealed preparations, and the recovered receipt/preparation-2 marker remain absent. Ordered Cargo gates reran successfully (2226 tests, one existing ignored); redaction, standards, Markdown, task-boundary and parity checks pass. Active IDs and archive/checklist/progress/README bytes are unchanged. This records the supported work and precise blocker; none of the three unresolved tasks is archived.

Reset-origin continuation | 2026-09-14: The owner authorized the recommended reset-origin investigation and progress-backed iteration. Preserve both startup failure seals and all existing judgments. Implement a separate prospective observation/recovery contract, with current validator identity distinct from the retained installed firmware daa69bb0 / ELF 49643f41 and its Gate pair. Initial reset category, actual observed boot transitions, advancing uptime and retained crash provenance are separate facts; unknown prior panic attribution remains explicit.

- [x] Add and regression-test closed reset-origin observations against the real retained capture, including first-marker panic history, actual in-window resets, regressions, missing coverage and conflicting identity.
- [x] Define and verify guarded observation preflight/serve/judge/review commands with historical task/context rejection, exclusive assignment, immutable evidence, no signing or pool inputs, and exact retained-runtime identity.
- [x] Implement and verify the missing idle authenticated Web Serial restart command and bounded expected-reset observation lifecycle before any restart effect.
- [x] Publish software/contract before effects; detect one Ultra 205 and capture 120 seconds of fresh authenticated read-only health/boot observations with fresh unchanged accounting. No installation is authorized by this observation step.
- [ ] Only after that observation passes, prearm continuous capture and consume one controlled software-restart request; prove the expected boot transition, exact identity, healthy startup and unchanged accounting, or preserve its failure and stop the current attempt.
- [ ] After independently accepted recovery, admit a fresh cadence preparation and repeat the full cycles/phases under unchanged mining, cadence and safety limits. Archive only on complete hardware evidence; otherwise retain the precise blocker and commit/push.

Recovery effects remain within the provided USB/barrel interfaces and sole browser Web Serial control. No old failed context or installation claim is reused, and an initial panic reason is never relabeled as known prior-firmware failure. A missing interface or observation gap must receive a prospective, verified correction before further effects; repeated authoritative signatures stop under the existing policy. All machine bounds, 2800-ms heartbeat enforcement, <=3000-ms revocation/shutdown, credentials/privacy rules, state preservation and 90/95 parity remain unchanged.

Stage-A software verification | 2026-09-14: [The prospective contract](docs/hardware/reset-origin-recovery.md) separates retained installed runtime from the current published host driver. Closed metadata is primed and deduplicated, with independent host/device spans, actual ordinal transitions, source conflicts, gaps and unknown prior-reset attribution. Forty-seven new tests are verified across canonical coverage and the actual retained-capture/Gate-parser replay. All 121 canonical targets and ordered Cargo gates pass (2226 tests, one existing ignored). A Bazel-only copied-file permission failure was corrected in the temporary fixture, then canonical testing passed. Ownership, reference, redaction, standards, Markdown and parity/progress checks pass. No firmware or Gate bytes changed. The existing interface supports observation but lacks authenticated restart and expected-reset framing; Stage B requires that separately verified addition. Publish Stage A before detector/browser use; no installation, restart, mining or cadence admission is authorized by an observation result.

Observation-001 preparation finding: Driver 881b8150 passed preflight and detector admission, but its HTML response was JSON-encoded by the shared response helper. The actual browser showed quoted element IDs and did not configure or open USB. Preserve `page-serving-failure.json` as the parent-observed first cause; the later server-close-before-end marker is cleanup chronology. The tab closed, supervisor exited 0 and process/USB cleanup passed. No accounting or observation ran. The targeted correction sends HTML as bytes and adds an exact real-HTTP response regression. A narrowly guarded `reset-origin-preflight --supersede-unstarted <failed-root>` may create a fresh observation root only after independent zero-activity/cleanup/failure verification and a changed clean published driver; retain the original root and assignment. Runtime bytes, observation limits and hardware authority remain unchanged.

Observation-002 admission verification: The exact unused failure is sealed as `c17d89b7451c54df7eadc94de79dde7734e4367c17a4ebbf3fe3b62fe12ac593`. Twelve context tests, eleven judge regressions and the real HTTP response regression pass; independent review found no actionable defects. Ordered Cargo checks pass (2226 tests, one existing ignored), all 121 canonical targets pass, and ownership/reference/redaction/standards/parity checks pass. Preserve the failed preparation and publish the corrected driver before allocating the exclusive observation-2 assignment. No new device evidence is claimed by these software results.

Observation-002 pre-capture finding: Driver 10334a45 reached fresh exact admission, idle preservation and before-accounting, then rejected the first diagnostic export with `reset_origin_preparation_receipt_review_required`. The closed Gate metadata reports `previous_boot/wrong_firmware` and `current_boot/unavailable`; no valid decoded receipt was observed. This is a missing host classification, not proof of a new device panic. No capture/start/end, after-ledger review, issuance or mining occurred. Worker Close and actual page/process/USB cleanup completed. Preserve this failed preparation and its missing proof. Before a fresh observation-3 assignment, verify the firmware meaning, retain strict closed receipt metadata without counting it as health or cadence evidence, keep valid/incomplete receipts blocked, independently seal the failure and implement a targeted successor guard with unchanged runtime/accounting/limits.

Observation-003 admission verification: Observation002 is sealed unverified at `e9d55dad8bb7bc837c5bc3507745cd8aee2be69a27b246a11fb48029f3003080` (35 original files, 13 artifacts, before-accounting only). The exact prior-marker regression failed before correction and passes afterward; the actual closed metadata replay and independent judge preserve the unknown attribution. Eighteen context tests and all 121 canonical targets pass; ordered Cargo gates pass (2226 tests, one existing ignored), as do ownership/reference/redaction/standards/parity checks. Independent review found no actionable defects. Per-invocation runtime-source deduplication reduced historical002 validation from 24.7 seconds to 16.4–16.6 seconds while preserving fresh-request and ancestor-tamper checks. Publish the corrected driver and retain every existing timeout before fresh observation003. No stability, recovery or cadence result is claimed.

Observation-003 journal finding: Sampling reached 130227 ms with no observed transition or diagnostic issue, but journal flush failed after 83 saved ready-state records; no end, after-accounting or closed journal row exists. Parent-observed Worker Close, page closure and actual host/USB cleanup are separate evidence. The preparation remains unverified at seal `bfa2376b1c58af97100b83755cc0964a321ef64801d3902a738a15f9a7fe2944`. Its failed transport cause was not retained. A no-hardware browser replay of the production client plus HTTP response helper reproduces rejection at request84: unconsumed keepalive response bodies leave the cumulative request-byte quota occupied. Implement response consumption with regression coverage, bind corrected host-client bytes separately from retained runtime artifacts, and add only a targeted sealed-failure successor after independent review. Preserve missing proof and require fresh complete capture/accounting under unchanged limits; do not relabel the failed result.

Observation-004 admission verification: The browser regression is sealed locally at `fc107819ea5ca2b94114d6e485cc207df8c1067da73bde930b7c42f7ef4c0906`, binding the exact response helper and before/after client hashes. Corrected client `2409804b08f6aa903d38e259baf527a7164c8329cd1bc43724f5632ce9cfa09b` completed 200 records in the same production-response replay. All six no-mining client tests and 24 context tests pass, including delayed/interrupted body completion, exact failure lineage, separate host-client binding and malformed attempt rejection. All 121 canonical targets, ordered Cargo gates (2226 tests, one existing ignored), ownership/reference/redaction/standards/parity checks and independent review pass. Publish before fresh observation004; no end/accounting/closed-row evidence is backfilled into observation003. No device recovery or cadence success is claimed.

Observation-004 finalization finding: The corrected journal completed 118 records and the browser closed normally, but independent review rejected `reset_origin_journal_order`: end sequence115 equals after-accounting sequence115, followed by closed sequence118. The observer omitted a fresh status publication after the end boundary. Preserve the complete original records and reviewer failure; do not relax the strict ordering predicate or claim an accepted observation. Add an explicit fresh status/flush before after-accounting, exercise both production clients against the real supervisor and independent judge with a fake device clock, and admit another observation only after a verified targeted correction and exact sealed-failure guard. Deduplicate the same charged predecessor within each fresh validation invocation so lineage growth cannot consume existing request deadlines.

Observation-005 admission verification: Observation004 is sealed unverified at `6c00926573200c8175809564cdc6833d5e875ff3a05e02b0aee40b8f0146a311`; its end115/after115 ordering failure remains intact. The complete production-client/supervisor/judge regression reproduces that rejection and passes with the fresh status/flush correction. Regression receipt `a79a8b9bc39b947a76b2269a446a46cdadaf5e5354c15caa713ff49b9c594935` binds corrected observer `dc15002f7c9e7dd8bf09bbd75afe61a79d70408961af1f7a1657a0994b1de525`; host journal client2409804 remains unchanged. All 122 canonical targets, ordered Cargo gates (2226 tests, one existing ignored), ownership/reference/redaction/standards/parity checks and independent review pass. The capture validator was extracted without changing its predicates. Per-request charged-predecessor deduplication reduced actual sealed-evidence validation to roughly five seconds, with fresh-request tamper rejection preserved. Publish before fresh observation005; no failed result is relabelled and no restart/mining authority is added.

Accepted Stage-A evidence | 2026-09-14: [Observation005](docs/parity/evidence/20260914-reset-origin-observation.md) passed the independent judge, canonical read-only revalidation and separate inventory audit. Result SHA `8847afd051a0a17ad3703dd3a5e6986b13e0c6ba395f705160095ad9f1543817` binds retained daa69bb0/ELF49643f41/Gate82992de9 to driver0179a6e5. Host coverage130039 ms, boot43 advances/126710 device ms, startup230 advances/128960 device ms, no transitions/conflicts/regressions/issues. Journal3→115→116→119 and both unchanged ledgers verify; final baseline/identity/settings/high-water, mine-on-boot false and actual host/USB cleanup pass. Prior panic attribution remains unknown. No restart, installation or mining occurred; observations001–004 stay unverified. Controlled-restart implementation and exact-pair recovery evidence remain outstanding before fresh full cadence qualification.


Stage-B installation 001 failure | 2026-09-14: Published a74701d6 / Gate e3f4ce62 / ELF a3f11024 completed the one admitted write, then reported `runtime_ready/complete/first_failure=statistics` in 49 samples. No controlled restart, post-install admission, allowance issuance or mining occurred. The canonical installation reviewer rejected `restart_install_identity_startup`. Browser, supervisor, three process observers, owned children, listener and both serial nodes are released. The 50-file failed inventory is sealed at `883b0a4f40eb97c59ec03f517d283d556e4ea3412de9e807e44371fdcf7ab3f1`; all previous evidence and markers remain immutable. Last authenticated accounting was before installation: next17, last16, 1380000ms, pendingfalse. Post-install accounting/preservation is unverified. Source review identifies returned statistics thread-spawn failure, not disk/NVS/schema access; matching-capability heap/error evidence is missing. INTERNAL|DMA|8BIT largest7936/free14651 before spawn is consistent with memory pressure but is not the exact INTERNAL|8BIT stack allocation class.

Statistics startup correction and bounded restart continuation | 2026-09-14: Continue under the owner's existing iteration authorization. Preserve the 8192-byte statistics stack and its scheduling; reserve its thread before startup heap fragmentation, hold it inactive until the existing statistics boundary, and cancel unused preparation. Add retained closed spawn/error/requested-capability and matching heap metadata without changing telemetry timing, mining, heartbeat deadlines or settings. No current failed context may resume.

- [x] Implement/test deferred statistics-thread preparation, activation/cancellation and retained matching-capability diagnostics; audit native costs and unchanged scheduling.
- [x] Add a narrowly guarded restart-preflight successor for the exact sealed installation001 statistics failure, preserving its consumed claim, original Stage-A assignment and missing post-install proof. Require verified correction, changed clean firmware, immutable lineage, one fresh installation/restart context and exclusive sibling assignment before creation.
- [ ] Require fresh authenticated current-image idle baseline/accounting and the bound known startup condition before the next installation; no mining, reset, factory/NVS alteration or failure reclassification is permitted by this preparation.
- [ ] Verify/publish the corrected implementation and prospective commands, build/audit the exact clean pair, then repeat the one-installation/new-pair observation/controlled-restart sequence. Admit full fresh cadence only through independently accepted restart evidence; keep normal180000ms reservation and all thresholds unchanged.


Statistics correction software checkpoint | 2026-09-14: [The failure/correction report](docs/parity/evidence/20260914-statistics-startup-failure.md) preserves the exact failed001 outcome and missing post-install proof. Ordered root Cargo gates pass (2239 tests, one existing ignored); all 130 canonical targets and ownership/reference/redaction/standards/parity checks pass. Deferred activation and retained diagnostics preserve stack size, sampling boundary and safety deadlines. Initial diagnostic code exceeded the 4MiB OTA partition by3248 bytes; removing unused private dynamic error formatting and redundant ownership wrapping preserves every failure check and yields4193968 bytes, with336 bytes remaining. Current native audit `436f7054998284bb5084374135cc89acae655610f311adef3da459bd9d6993c5` binds ELFf587f219, selected statistics path5936/8192, main telemetry12560/16384, owner entry4512/8192 and static diagnostics1320/2048; these are selected-path bounds, not full callgraph or hardware proof. Recheck the clean published package before effects.

Environment blocker | 2026-09-14: The updated Gate passes605 WebCrypto tests, full TypeScript/browser/package/standards checks, and Cargo format/Clippy/build. Its Rust suite stopped on two Docker container-creation timeouts; later suites and headless verification remain incomplete. Docker Desktop's API/status are unresponsive and its normal restart timed out while the backend remains stopping. Updated Gate publication is therefore blocked. The firmware pin remains published e3; the new preflight explicitly rejects that bundle's missing statistics decoder before assignment, preventing hardware use with an incomplete pair. No successor002 context, installation, restart or mining reservation has been created. The owner was asked whether Docker carries other work before any forced shared-service interruption. Cadence, qualification and migration remain active; parity is90/95.


Verification resumption | 2026-09-15: The owner explicitly authorized forcing Docker Desktop to restart. `docker desktop stop --force --timeout 45` and `docker desktop start --timeout 60` completed; the engine API responds with version29.3.1. Gate TypeScript and all605 WebCrypto tests pass again. Ordered Cargo format and Clippy pass, but build now fails at Apple's selected `/Applications/Xcode.app/Contents/Developer` toolchain with exit69: the Xcode/Apple SDK license has not been accepted. No license was accepted on the owner's behalf and no toolchain bypass was attempted. Native verification, Gate publication/pin update and successor002 hardware remain blocked pending owner review/acceptance of that agreement. No new hardware context, restart or mining allowance was created; previous seals and accounting evidence remain unchanged.


Environment recovery and pair publication | 2026-09-15: Docker's authorized forced restart restored its engine API; the owner then accepted the Xcode license. Gate's complete ordered Rust suite passes (352 tests, two existing ignored), with 605 WebCrypto tests and all TypeScript/browser/headless/package/standards checks passing. Gate72235d884c3605ff77d484f210fcaa0d7218b47e is published with retained statistics diagnostics. Pin the verified Gate, finish firmware-repository verification, publish/build the clean exact package, then use the already contracted successor002 command. Hardware remains unverified; failure001 and all predecessors remain sealed. No new allowance has been issued.

Successor002 publication checks | 2026-09-15: Ordered firmware Cargo gates, all130 canonical targets, ownership/reference/redaction/standards and parity/progress checks pass against Gate72235d8. Publish the pin and task record, then build/audit the exact clean package before preflight. Original failure001 and its accounting/cleanup limitations remain unchanged; no successor effect has occurred.


Required-thread reservation and guarded successor003 | 2026-09-15: Successor002 used published firmware582fc2bf/Gate72235d8/ELF8095cfcf. Fresh authenticated pre-install accounting and the bound old statistics failure passed; the single ordinary write then completed. Statistics preparation and activation succeeded, but startup reported `wifi_startup_failure phase=reconnect_spawn error=no_memory` and `runtime_ready/complete/first_failure=network`. Canonical installation review rejected `restart_install_identity_startup`; no post-install authenticated preservation/accounting, controlled restart or mining occurred. Last authenticated ledger remains next17/last16/1380000ms/pendingfalse. Actual browser/supervisor/child/listener/USB cleanup passed. Preserve the failed capture and incomplete supplementary supervisor observation; the separate mandatory installation observer completed with 281 observations, no failures and no remaining processes.

Continue the existing authorized iteration with the complete fixed-owner allocation boundary, following the native USB ownership and memory-profile rules. Reserve the unchanged 8192-byte reconnect thread during Wi-Fi preparation and the unchanged 8192-byte USB receive thread during Worker preparation. Reuse the statistics activation gate, retain each existing activation boundary, and cancel unused preparations. No early network-event processing or USB reads, stack reductions, cadence/priority changes, deadline changes, new wire commands, partition changes or implicit NVS effects.

- [x] Independently seal/audit successor002 as an unverified reconnect-thread allocation failure; preserve its actual legacy outcome, full lineage, all 13 artifacts, consumed claim, missing post-install proof and cleanup limitations.
- [x] Implement/test required-thread reservation and activation at the production call sites, including fallible spawn, queued events before activation, cancelled/failed preparation, subscription cleanup and no USB access before activation. Audit remaining late allocations, native stack/storage costs and exact image fit.
- [x] Extend the existing restart preflight only for the exact sealed002 failure: changed clean/published firmware, verified correction, exclusive assignment3 before child creation, fresh context and current002 runtime identity. Require fresh authenticated unchanged accounting and same captured boot/reset with advancing known network failure plus active statistics before the one installation. Preserve the001 reader and all exhausted/archived authority rejection.
- [ ] Run ordered Cargo gates, affected supervisor/Gate regressions, canonical tests, native packaging, ownership/reference/redaction/standards/parity checks; review and publish before any new effect. Keep Cargo and Bazel separate.
- [ ] Execute only the fresh successor003 contract: detector admission, one state-preserving install, healthy new-pair observation and one authenticated controlled restart, followed by actual cleanup and independent review. A failed or consumed context cannot restart. Only accepted restart evidence permits fresh cadence17-2 with four cycles and the unchanged normal 180000ms reservation and cadence/shutdown criteria.

Qualification and migration remain active; parity remains 90/95. Conditional captive-DNS and explicit reconnect-probe allocations remain separate paths requiring their own evidence; this correction does not qualify them.

Failure seal: [the exact successor002 report](docs/parity/evidence/20260915-reconnect-startup-failure.md) binds inventory SHA `29c8f6bb14b1962676bdd8af4a5c9b406ea81ab0f68c61882c51c2680eb0d7ad`, producer `4946a65aaa619c0dd291ca09932cadaa87ed2c7f1eccb7507cf8ef2a34e5efab`, 51 files and 15 directories. Boot1/other and last boot/startup uptimes29751/29752 ms are observed context for the next pre-install review, not a cross-image continuity claim.

Image-size verification replan | 2026-09-15: The initial required-thread reservation preview compiled but produced4209040 bytes, exceeding the unchanged4194304-byte OTA partition by14736 bytes. Sharing one compiled startup callback path and fixing AP-only conditional reservation reduced the next image to4199504 bytes, still5200 bytes too large (ELF `f230095e4610908f12719939de32b72a60e81ea6df83c3647abea12dd4b3e4b6`). Both host-recorded packaging attempts exited cleanly with the size rejection and no effects. After two failed size checks, pause implementation edits and compare actual native symbol/section growth to select a measured bounded reduction before another build. No feature, acceptance guard, stack or partition may be removed/reduced to claim success.

Measured size experiment | 2026-09-15: Restoring `event_from_inbox` as an explicit non-inlined boundary reduced the preview by1648 bytes to4197856, still3552 bytes over the fixed partition. Its ELF is `742199ab2eb03882c033f20639180d8f99b967a3a6fd81b237245d9da051e5e6`; owner entry frame5264 remains within8192, mapper336 and wake-event576 are separately observed frames, not a complete callgraph claim. The next bounded experiment encodes prepared station credentials and their required reconnect worker in one enum variant, removing the representable missing-worker state while preserving AP-only zero allocation and association fallback. Canonical verification currently reports130 passed tests and three packaging-dependent skips; size rejection remains the blocker. Ownership/reference/redaction/standards pass and parity stays90/95.

Second size replan | 2026-09-15: Encoding prepared credentials removed the impossible state and passed its regressions, but saved only96 bytes (4197760-byte image,3456 over). Retain the representation improvement without claiming a size solution. Pause edits after the two measured boundary/representation experiments and evaluate a narrowly scoped cold annotation for the one-shot startup routine, whose inlined initialization body is about20KiB. Require a new size result and native stack audit; do not alter global optimization flags, runtime loops, features or acceptance limits.

Cold-path experiment | 2026-09-15: The isolated startup cold hint left the image unchanged at4197760 bytes,3456 over. Remove that ineffective annotation and preserve its private build evidence. Assess a single static dispatcher for the three fixed prepared owners as the next bounded representation change, replacing the boxed callback without changing stacks, activation gates or runtime loops. No further hardware effect is eligible while packaging fails.

Package-profile replan | 2026-09-15: The closed dispatcher increased the image to4204048 bytes, so restore the smaller boxed implementation, prepared-credential enum and measured mapper boundary. The existing release profile already scopes size optimization to `bitaxe-firmware`. Test only that package's `opt-level` from `s` to `z`, using Cargo's supported package override; dependencies, global profile, C/ESP-IDF flags, partitions, stack sizes and safety/cadence values remain fixed. This is a measured revision of the earlier implementation-choice restriction, not an acceptance-limit exception. Cargo documents that neither size setting guarantees a smaller result. Require exact native image and main/owner/statistics/RX/reconnect frame audits, then all software checks, clean publication and the unchanged full hardware qualification. No hardware context may be created from a failing package.

Successor003 pre-publication verification | 2026-09-15: The package-scoped `z` profile produces4118992 bytes with75312 bytes remaining in the unchanged4MiB slot. Ordered Cargo format/Clippy/build/tests pass (2239 tests, one existing ignored); all133 canonical targets pass, as do Gate605 WebCrypto tests/typecheck and standards/ownership/reference/redaction/parity checks. The exact outlined publisher audit extension passes18 tests, including oversized/clobbered/unknown/dynamic-stack rejection. Independent reviews found no remaining actionable findings after the AP-only reservation correction. Private native audit SHA `ceabd7d69d723c330bf89e12a0ca215d5bd257a1814e950a8ae81f5a2f7fe992` binds preview ELF `5f14c644ea27b2d1321ca79ac6a777ba833e2114d11862760143bcb69a227485`, profile/source/tool hashes, main12448/16384, production entry5024/8192, statistics6960/8192, RX2224/8192, reconnect944/8192, control owner8224/16384 and diagnostic storage1320/2048. These are selected-path costs, excluding unproven library/ROM/allocator/interrupt depth; hardware safety/timing are not established. Publish this reviewed snapshot, build/audit the exact clean package and only then create successor003. Failed002 remains sealed at29c8f6bb; no new installation, restart or mining reservation has occurred. Parity remains90/95.

HTTP task allocation correction and successor004 | 2026-09-15: Successor003's one write on253658ce/Gate722/ELF6991aab2 completed, then failed with `http_server/http_task` and `runtime_ready/failed/first_failure=storage_http`. The canonical installation reviewer rejected it. Statistics reached active; no post-install authenticated accounting/preservation, controlled restart or mining occurred. Last authenticated accounting remains next17/last16/1380000ms/pendingfalse. Actual browser/supervisor/children/listener/USB cleanup passed; the separate supervisor observer expired incomplete at600s and is not cleanup proof. The immutable failure seal is `d560b740bcca4936dd6c39722a81165112decc5579ce2c4f78d8d588903c855c`, producer `7e90435786f3ab61594991757863ed8d7f3eae4e841e0d2252c6416e2c33acac`, with51 files/15 directories and13 runtime artifacts.

The pinned IDF path returns this category when stack/TCB allocation for the16KiB HTTP server task fails. Current HTTP initialization starts its8KiB deferred worker first. Test reserving the larger HTTP task before that smaller worker within the same startup stage, keeping routes/readiness publication after both succeed and releasing the server if deferred startup fails. No stack, Wi-Fi buffer, reserve, priority, affinity or deadline change is planned; the exact failed suballocation and matching-capability heap at failure remain unmeasured.

- [x] Verify the sealed003 evidence independently and implement/test the production HTTP owner-order boundary, including fragmented-heap reproduction, no deferred start after server failure, server cleanup after deferred failure and unchanged error provenance.
- [x] Add only an exact sealed003→successor004 guard with immutable ancestry/assignments, changed clean published pair, fresh authenticated unchanged ledger and the same captured boot plus active statistics and exact failed HTTP startup. Preserve incomplete/missing proof as failure; prior readers and archived-task rejection remain intact.
- [ ] Run ordered Cargo, affected/canonical/Gate checks and fresh native package/stack/storage audits; review and publish before effects.
- [ ] Run one fresh bounded installation/observation/controlled restart only after that preparation passes. Full fresh cadence17-2 remains gated on an independently accepted restart result, with unchanged four cycles, normal180000ms reservation and all timing/shutdown criteria.

Successor004 verification checkpoint | 2026-09-15: Ordered Cargo gates pass; the production HTTP helper passes three behavior tests and reversing its actual order in a private mutation fails. All135 canonical targets passed across the full run (134pass, one historical fixture timeout) and unchanged isolated rerun (90.3s within300s); the original timeout log is retained. The64 affected host regressions and actual Gate parser check pass. Independent firmware/guard reviews found no actionable defect. Native preview audit `fcdcd74e68bcf10e78f30c73f0449a6460d81dbba5632fce21088ea0d8d9c454` binds ELF `9d518f8d6c3979bcf5f8f43c1539b85b9127d0b6fd3d5988da0e3b78f8fa986c`,4119008-byte image/75296-byte headroom, main12448/16384, statistics6960/8192, RX2224/8192, reconnect944/8192, control8224/16384, production entry5024 and storage1320/2048. HTTP server/deferred setup paths are2736/2448 within main16KiB. A private auxiliary startup span uses exact ELF function extent to exclude literal-pool data; no threshold or canonical parser was relaxed. These are selected native paths, not whole-task or hardware guarantees. Publish this snapshot and verify the exact clean package before reserving004; prior seals and budgets remain unchanged.

USB cadence retention contention correction | 2026-09-15: Fresh cadence017-preparation2 onF056/Gate722/ELFb47 passed five healthy installations and four complete continuity cycles. Idle passed100/100 intervals; USB completed all12 maximum probes but only89/95 intervals were<=750ms (93.6842%, below95%), with maximum828450us and processing320724us. No publication, capture, CPU, priority or subscriber errors occurred. Mining stayed empty; no allowance was issued/consumed and no fault was armed. The last observed cooling ledger is17/16/1380000ms/pendingfalse; no fresh post-failure ledger is claimed. Complete USB review remains in journal34 through closed39; canonical USB phase/result files remain absent. Parent-observed browser error is cadence_supervisor_rejected during USB finish. Actual browser/server/passive-observer/children/listener/serial cleanup passed; supplementary supervisor observation expired incomplete and supplies no cleanup proof. Failure inventory `2003dd26039daa1edad78057e88215a07fffab4883d955648b8a61ad12f8aacb`, producer `f894edb393c176edc115e5408530a6ebe1045709f1b17543ca91450e19395c78`, preserves115 files/16dirs and all prior outcomes.

The worst interval decomposes exactly into320724us processing plus507726us gap. Retention accounts for153720us in that iteration (maximum163131us). Source review finds USB diagnostic replay scanning up to512KiB under the same log mutex required by atomic pair retention, including recurring no-match scans. This is the leading contention hypothesis, not yet a measured lock-wait attribution.

- [x] Cache the seven numeric boot memory checkpoints and first closed BWG startup failure at their typed producers; replay these immutable boot facts without accessing the circular log mutex. Keep log downloads and atomic operator-pair retention/issuance unchanged; document boot-lifetime replay independent of log eviction. Target116 additional static bytes, remaining below2048 total.
- [x] Test actual replay while the log mutex is held, first-value/boot-reset semantics, concurrent publication, unavailable/corrupt cache handling, exact closed formatting and unchanged retention/issuance failure guarantees. Preserve no waiting locks/allocations/logging in cache recording and unchanged heartbeat service priority.
- [x] Define/test a finite exact failed-USB-cadence successor for fresh ordinal17 preparation3, with new clean published runtime, preserved ancestry/assignments/ledgers and fresh authenticated accounting. Do not transfer prior software-restart or cadence success to the changed image. No old classifier or threshold may be weakened.
- [ ] Run ordered software/canonical/native storage/stack/package and privacy/ownership/parity checks, review/publish the corrected pair, then repeat the fully bounded qualification under its new prospective contract. All four cycles, all three captures and independent shutdown/cooling remain required before closure.


Software correction verification | 2026-09-15: The real old replay reader failed while the retained-log mutex remained held; the corrected reader passed all eight slots, including absent entries. Nine cache, eleven replay and nineteen unchanged retention/publication tests pass. All53 affected/historical host tests pass, including missing/altered artifacts, cleanup/failure contradictions, work/renewal/fault evidence, ledger drift, recursive lineage, duplicate assignment and interrupted creation. The read-only failure command independently revalidates the actual115-file seal and retains post_failure_ledger_observed=false. Ordered Cargo passes2239 tests with one existing ignored test; Gate passes605 tests. Native preview ELF `8e1e36fa12080213bd533412e96d3abf4d006f500d1e945876b79856a1086dba` uses4118160 app bytes,1436 diagnostic static bytes and a12560/16384 selected main stack path. Audit `16d97887004d1c2fee4d6a3f24d907207464bb7dd75df86d4503803d4137d4fb` and independent byte-level recording audit `7ae7d170da3e32bdbc28f20b9e21dc5ad97e1430ec473e2db1cc9d31dc244a2e` preserve their stack/library limitations. Each typed cache recording section has one inline hardware CAS, no calls and no backward branches; surrounding formatting/logging still allocate. No new hardware context or effects have been started. The full corrected canonical graph passes all138 targets with four concurrent tests and unchanged per-test limits. Its earlier timeout and isolated getter assertion failure are retained; removing the shared-reader wrapper restores historical invocation behavior. Ownership/reference/redaction/standards checks pass; parity stays90/95 with archive/checklist/progress bytes unchanged. Publication and the clean exact package remain before preparation3 admission. See `docs/parity/evidence/20260915-usb-cadence-retention.md`.
...
- [ ] After accepted Noise evidence, create a separate immutable plan.
- [ ] Re-prove TCP and authenticated Noise, then complete setup, channel
      opening, target receipt, and job receipt against the local fixture.
- [ ] Restore recovery-006 exactly and independently validate the closed
      projection, cleanup, and redaction.
...
task-local hardware namespace begins at `session-001`. ASIC work, qualified
nonce generation, share submission, and mining remain excluded.
...
- [ ] After accepted channel/job evidence, create a separate immutable plan.
- [ ] On one exact package, prove TCP, Noise, setup, channel, target/job,
      BM1366 work, qualified nonce, encrypted accepted share, safe-stop,
      cleanup, and exact recovery-006 restoration.
- [ ] Independently validate the cumulative hardware projection and redaction.

Depends on: completed `task-str005-v2-channel-job-205` with accepted evidence.

Authorization: software work only until its immutable plan exists. The final
campaign retains global STR-005 ordinal `attempt-008`, the local authenticated
fixture, 400 MHz, 1100 mV, 100% fan, and the 180-second safety ceiling. Earlier
attempt history must not be reset. External pools and every previously excluded
hardware effect remain prohibited.
...
- [ ] Create `RESULT.md`, transition only STR-005 to `verified` with
      ...
      archive the completed child tasks.

Depends on: completed `task-str005-bm1366-share-205` with an independently
accepted full-share campaign and exact restoration.
