...
- [ ] Verify and publish coordinated diagnostic/protocol changes before a new bounded hardware attempt; keep all earlier failures and the one campaign budget intact.
...
- [ ] Qualify the documented ESP-IDF memory-saving Wi-Fi buffer profile (static RX/TX 6/6, receive window 12, dynamic RX 32) for the measured allocation failure, accounting for retained HTTP and later USB/statistics owners. Preserve all stacks, the internal reserve, safety owners and scheduling.
- [ ] Reject stale/missing/duplicated resolved buffer settings before build artifact publication, restore the omitted build-validator test to the canonical suite, remove its obsolete TinyUSB fixture, and repeat required checks and clean packaging.
- [ ] Require healthy startup before the next browser probe; retain RX diagnostics for any subsequent transport failure.
...
- [ ] Prove qualified shutdown, bounded cooling, cleared volatile secrets/leases, persisted mineonboot=false, new firmware installed and all resources released.

Authority: the user explicitly selected the existing owner pool and these three bounded live windows. This narrowly replaces the earlier no-mining/external-pool prohibition for this task only. Use the existing Conservative Ultra 205 profile: 400 MHz, 1100 mV, fan 100%, fresh observations, input 4.5–5.5 V, power at most 15 W, ASIC temperature below 75 C and fresh nonzero fan RPM. Owner pool TCP/Stratum V1 access is allowed only from the protected configured input. Firmware grant inputs remain volatile; normal updates preserve existing NVS secrets and replay state.

Budget: one campaign identifier minted and sealed once; reserve each full 180000/30000/30000-ms window durably before enabling work, never refund an interrupted reservation, and never mint a replacement campaign automatically. A device-local active-time ceiling is independent of signed lease renewal and the 2.8-second link deadline. Count from first work dispatch until confirmed ASIC reset/power-off, and reserve the bounded pre-reset shutdown tail before closing work admission. Preparation time is not a substitute for that bound. Missing accepted share at the ceiling is unverified, not permission to extend.

Recovery: revoke submissions/work, close owned pool transports, execute the existing ordered ASIC reset/frequency-down/voltage-off/enable-off sequence; retain fan 100% until fresh temperature is at or below 45 C, then 30%, under the existing 120-second cooling bound. Preserve the earliest failure. Any fault or missing proof fails closed. Only regression-backed progress admits a new bounded attempt, within the same campaign budget.

No electrical fault injection, direct UART/pins/probes, erase-flash, arbitrary raw writes, unrelated network discovery, foreign process termination, or automatic parity promotion. Intentional heartbeat suppression is a test-harness transport action, not an electrical fault. Protected mode-0600 evidence under mode-0700 parents contains only sanitized operational material; published projections contain closed facts/counts and safe provenance, never secrets or identifiers.

Attempt-008 first Start failure (2026-09-05): normal window 0 was issued and its memory-only delivery consumed once. Start ended with browser `start_failed/liveness_lost`; firmware reported the secondary RX stage `session_revoked`. No running or qualification-generation sample was produced. Fresh reconnect confirmed restoration, inactive lease, mine-on-boot false, and unchanged Device Identity, settings and durable authorization marks. Browser streams/locks were released, the exact supervisor exited, and both serial nodes and the supervisor listener have no holder. The signed window and all nine public samples remain immutable; no lease retry or later window ran. No active mining is evidenced, and no reservation is refunded or replacement campaign authorized.

Progress-backed correction plan:
...
Owner-authorized completion plan (2026-09-06): ADR-0024 and Gate ADR-0097 define the successor criteria without refunding consumed reservations.

- [x] Reproduce and close pre-owner reservation cleanup, pending-write revocation and competing finalizer races; prove real owner queue rejection and durable failure handling.
- [x] Add nonblocking closed admission diagnostics, correlated closed errors and possession-bound read-only original-campaign budget review.
- [ ] Run complete Gate/firmware verification, publish Gate, pin its exact archive, publish firmware and build its exact clean ESP32-S3 package.
- [ ] Create fresh attempt-011 using the existing protected campaign parent, prove no serial/supervisor holders, detect the same Ultra 205 and install only admitted disjoint NVS-preserving segments.
- [ ] Qualify four current-image no-mining cycles with 65536-byte request/response probes, exact identity, settings/authorization preservation and cleanup. Test invalid-signature Start rejection and fresh reconnection with unchanged reviewed ledger; no valid lease reservation is permitted for this negative test.
- [ ] If the fresh bound ledger proves only original window 0 charged/completed, use `create-successor` to bind attempt-010's immutable failure and the new context/cycles/review. Then perform original windows 1 and 2 with 5000-ms signed renewal intervals and fresh review before each. Original normal-window success remains unverified.
- [ ] Record real work/renewal/share and each fault timing only if observed within the original remaining reservations. On any failure restore, cool under the existing bounds and retain earliest evidence; no unchanged retry, replacement campaign, refund or ceiling increase.
- [ ] Verify final safe new image, persisted mine-on-boot false, inactive leases, private-input cleanup and released resources; record reviews and publish evidence.

Effect contract: repo-owned `fixed-usb-qualification preflight/serve/record-cycle/create-successor/judge`, `detect-ultra205`, and `flash-monitor --board 205 --manifest <exact-clean-package> --image <declared-image> --port <detected-port> --evidence-dir <fresh-private-child> --redact-evidence --capture-timeout-seconds 30`. Retain physical lease and ROM admission before writes; each CLI process is bounded and reaped before browser reuse. Existing ignored owner-pool input and protected role-separated authority directory may be consumed only by the original signing path; no values enter logs. Source/requirements publication precedes hardware. Fresh no-mining rejection uses only a synthetic invalid signature proven to fail before any Worker session effect. Live effects retain 400 MHz, 1100 mV, fixed fan 100%, fresh 4.5–5.5 V / at most 15 W / below 75 C / nonzero-RPM observations and three-second authority closure/shutdown initiation. Ordered reset/voltage-off/enable-off and bounded cooling to 45 C or below before fan 30% remain mandatory. Only reviewed original unreserved windows may be used; unknown or different budget state stops for evidence rather than silently expanding authority.

Pre-owner completion verification (2026-09-06): Gate `b877570b31dba3c0a743e88aed3f3b4d02e0aa4f` is published and pinned with archive SHA-256 `ab9d5d616093d2878592aba604a89298dbda03e528ef4c03d6d1b50cbd15d112`. Its full verification passes, including 371 web/CLI tests, Rust, TypeScript, browser conformance, package and standards. Firmware ordered Cargo gates pass (2092 tests, one preexisting ignored). The Bazel run passed 82 of 83 targets; automation child fixtures hit their unchanged deadlines during overlapping builds. After those builds ended, the same isolated automation target passed in 76.1 seconds. The original failures remain retained; no deadline or assertion was relaxed. Final exact-pin conformance/package and ownership/redaction checks precede firmware publication and hardware effects.

Final pin checks (2026-09-07): all 15 affected pinned conformance/owner/cleanup/supervisor targets pass. Actual ESP32-S3 packaging, fixed-USB ownership/symbols, reference integrity and redaction pass. The original failed automation run is preserved alongside its passing isolated rerun. Attempt-011 is unused; the final clean post-publication package remains the mandatory preflight input.

Attempt-011 review (2026-09-07): initial clean install and all four current-image cycles passed; fresh possession, actual 65536-byte request/response exchanges, preservation and cleanup are proved. The invalid-signature no-mining rejection/reconnection check passed with unchanged original ledger (180000 ms/masks 1). Original window 1 then failed before preparation: readiness mask 55, fresh zero fan RPM, zero active milliseconds and zero work. Fresh recovery succeeded and the original ledger now proves 210000 ms/masks 3/no pending reservation. No fault was triggered and no share or physical shutdown timing is claimed. Resources are released and the cdbf47e0 image remains safe. See [attempt-011 evidence](docs/parity/evidence/20260907-fixed-usb-attempt-011.md). Its first failure and all consumed artifacts remain immutable.

Final-window plan (2026-09-07, ADR-0025 / Gate ADR-0098):

- [x] Reproduce/fix initial fan-readiness circularity without relaxing running-state RPM checks; prove post-acknowledgement RPM precedes every power/ASIC effect.
- [x] Add fan-only qualification and safe restoration through existing owners, with no mining reservation or ASIC capability, journaled cleanup and queued temperature/generation checks.
- [x] Preserve typed ordinary Start rejection in Gate; retain attempt-011's original generic observation unchanged.
- [x] Verify/publish Gate, pin exact archive, verify/publish firmware and build the exact clean package.
- [x] Fresh attempt-012 under the same protected campaign parent: cleanup/detect, state-preserving install, exact safe baseline and four current-image cycles with preservation and maximum exchanges.
- [x] Use the qualified browser fan-only review: full-fan acknowledgement plus fresh post-command RPM, restored 30% only with fresh temperature at or below 45 C, and equal original ledger before/after. Do not issue a lease if this fails.
- [x] Bind a version-2 successor to attempt-011's immutable ancestry/recovery/ledger and the new cooling/cycle receipts using `create-successor --predecessor-root <attempt-011> --input <budget-review> --cooling-input <cooling-review>`.
- [ ] Use only original window 2 (30000 ms, 5000-ms renewals, 15550-ms shutdown reserve). Observe work/renewal/share where possible, then suppress heartbeats with more than 3000 ms fresh headroom. Windows 0/1 remain consumed/unverified; no refunds or new campaign.
- [ ] Complete qualified shutdown/cooling and final ledger review, leave the new image safe with mine-on-boot false and resources released; publish final evidence and unresolved criteria honestly.

Effects retain the existing task's exact detector/segmented flash/supervisor commands, same physical lease and ROM admission, protected inputs/evidence, 30-second no-mining observer, conservative 400 MHz/1100 mV/fan 100% profile and all fresh electrical/thermal/RPM limits. Fan-only qualification cannot issue voltage, ASIC, frequency or pool effects. No further mining beyond the final original reservation is authorized. Any failure preserves its first signature; no unchanged hardware retry, reset of accounting, electrical fault injection, direct UART/pins, unrelated discovery or parity promotion is allowed.

Final-window software progress (2026-09-07): Gate `c8ffb63420b0c7e5e6fed072469a9bcda4cd5194` is published with full verification (380 web/CLI tests plus Rust, TypeScript, browser, package and standards); archive SHA-256 `58a5a45aa942ad098dd41fdd08baee8d0dde29c311cfabf4856a047e8867d40a` is pinned. Firmware ordered Cargo checks pass (2097 tests, one preexisting ignored). Focused fan/owner/cleanup and controller tests pass, including post-ack RPM freshness, boot-restoration gating and no budget/ASIC effects. New final-window/cooling receipts and bounded ancestry are checked by 40 passing supervisor tests. All 84 pinned Bazel tests now pass, including real ESP32-S3 packaging; fixed-USB ownership, reference, redaction, standards and scoped formatting checks pass. SAFE10 current projections use v2 with all 23 source paths while historical v1 retains its exact 19-path contract and CFG07 digest. Earlier source-inventory failures remain preserved. The unrelated existing TASKS.md formatting is unchanged; the active block and new documents pass scoped checks. Publication and an exact clean post-commit package precede attempt-012.

Attempt-012 pre-live review (2026-09-07): exact clean firmware `a90b436e` / ELF `c43fd085` with Gate `c8ffb634` passed initial installation and all four maximum-exchange, preservation, update/reconnect and release cycles. Fan-only qualification proved fresh post-command 2361 RPM at full fan, acknowledged qualified restoration to 30%, and unchanged original ledger 210000 ms/masks 3/no pending reservation. Last-window successor creation failed first as `successor_baseline`: its historical recovery validator incorrectly demanded an open connection for attempt-011’s correctly released final record. No lease was issued or consumed. Safe restoration, browser serial release and supervisor cleanup are confirmed.

Qualification-only correction plan:

- [x] Require released safe recovery for the historical predecessor, while preserving connected, fresh possession-bound admission for the current successor; test both directions and unsafe/mismatched records.
- [x] Add a version-2 qualification amendment for an already-four-cycle context. Bind the unchanged context, exact runtime snapshot, four receipt hashes, unchanged Gate/ELF/campaign/limits and the published qualification-tooling commit. Permit only scoped qualification source/task changes; reject runtime drift, altered evidence or any issued window. Preserve legacy amendment v1 and all original records.
- [x] Verify and publish the correction, seal the repo-owned `amend-policy` receipt, restart the supervisor against the exact snapshot, reconnect the existing qualification page for fresh possession and ledger review, then create the v2 successor and execute only the last original window under ADR-0025. The firmware and its verified four cycles are unchanged; no replacement campaign or budget refund is permitted.

Qualification-only software verification: reproduced both historical-baseline and current-context amendment failures before the fix. The supervisor suite and canonical Bazel qualification target pass; ordered Cargo checks pass (2097 tests, one preexisting ignored), as do standards, scoped task formatting and diff review. No runtime source or Gate artifact changed.

This amendment is an explicitly authorized qualification-tool correction within the existing task. It creates no USB, mining or settings effect itself. All physical-effect, privacy, recovery, cleanup, timing and final-state constraints above continue unchanged. Preserve the failed successor command as the earliest harness failure.
...
Execution plan | 2026-09-10: Reproduce the stale-complete-reply boundary in
Gate's production Web Serial harness; implement the smallest bounded bootstrap
correction with adversarial identity/deadline/cleanup regressions; verify and
publish Gate, then consume its exact archive in firmware. Define and publish
the no-mining qualification contract before device effects, run four exact-pair
continuity cycles and fresh-session recovery, and seal redacted results. Review
the diff and archive this task only after its exact acceptance criteria pass.
...
No-mining hardware contract | 2026-09-10 | hello-recovery-001

Objective: qualify the published bounded browser Hello recovery with fixed
Serial/JTAG on exactly one admitted Ultra 205. The Gate runtime changes, so a
fresh exact firmware/Gate pair must pass four no-mining update/reconnect cycles.
Preserve the installed Device Identity, settings, authorization high-water
marks, original exhausted campaign, and qualification-attempt ledger. This
contract grants no mining or signing allowance and supersedes no old evidence.

Before effects: complete ordered Cargo gates, Gate verification, canonical
Bazel tests/package, native USB ownership, reference, redaction, standards and
diff review; publish Gate, pin its exact archive, then publish firmware/task
contract. Build `just package` at that clean pushed firmware HEAD. Resolve the
Gate commit from MODULE.bazel and require its clean pushed checkout and bundle.
Create mode-0700 `scratch/hello-recovery-20260910`; the supervisor exclusively
creates its absent `attempt-001` child. Detector/flash output uses distinct
mode-0600 sibling logs. Existing original campaign input is read-only:
`scratch/fixed-usb-qualification-20260904/campaign.json`; never print its value.

Commands, with `FIRMWARE_SHA` and `GATE_SHA` bound to those exact published
commits and `PORT` taken only from fresh same-device admission:

Resolve path arguments absolutely from the firmware repository before launching
Bazel; relative paths below identify the repository-owned locations.

```text
just detect-ultra205
just fixed-usb-qualification no-mining-preflight --firmware-root /Users/peterryszkiewicz/Repos/bitaxe-esp-miner --gate-root /Users/peterryszkiewicz/Repos/bitaxe-turnstile-system --firmware-commit FIRMWARE_SHA --gate-commit GATE_SHA --manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --private-root scratch/hello-recovery-20260910/attempt-001 --original-campaign-record scratch/fixed-usb-qualification-20260904/campaign.json
just fixed-usb-qualification no-mining-serve --private-root scratch/hello-recovery-20260910/attempt-001
just flash-monitor --board 205 --port PORT --manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --evidence-dir scratch/hello-recovery-20260910/install-001 --capture-timeout-seconds 30 --redact-evidence
just fixed-usb-qualification record-cycle --private-root scratch/hello-recovery-20260910/attempt-001 --input PRIVATE_CYCLE_REPORT
just fixed-usb-qualification no-mining-judge --private-root scratch/hello-recovery-20260910/attempt-001 --input PRIVATE_REVIEW
```

The task-specific 30-second flash-monitor capture is a bounded no-mining
safe-baseline smoke using the existing exact-source/ELF and advancing-runtime
classifier; it does not replace general 360-second bring-up evidence. Use
fresh `cycle-001` through `cycle-004` evidence directories for the four later
same-package flashes. Every write requires same-physical-device ownership and
successful ESP32-S3 ROM board-info. Admit the existing validated disjoint update
segments only; never provision or erase NVS. Standard flash-owned application
return/reset is allowed once per write; no standalone reset or reflash retry
is authorized merely because browser admission failed.

Browser/evidence sequence: use only the served no-mining page in macOS Chrome,
with the original campaign identifier obtained inside that local page through
the read-only supervisor route. Record both ledgers before/after, retain the
page's private preservation baseline, await completed Connect, record exact
identity and inactive/restored/mine-on-boot-false state, and run the 65536-byte
probe. Release streams/locks and prove no unexpected USB holder before every
flash. Reconnect and record each real cycle against the same baseline. Preserve
all original command/browser samples; validators alone are not physical proof.

After four cycles, exercise one baseline foreground-loss disconnect while a
read-only refresh/probe is pending, and one cancellation during fresh Connect
using the existing Close action. These are no-work transport interruptions;
never load/start a window, suppress heartbeats during work, or request ASIC,
fan/voltage, pool, provisioning or network-discovery effects. Record actual
visible/hidden transitions, completed closure and a fresh explicit Connect.
Require at least one observed stale complete record discarded within 32 records
and 66560 skipped wire bytes, followed by completed exact-identity possession,
safe restoration and unchanged accounting. If no stale backlog occurs, retain
the valid continuity evidence and report that hardware criterion unverified;
do not manufacture or inject device records or blindly repeat the fault.

Recovery/stop: after an interrupted admission, await owner cleanup, retain
provided USB/barrel power, and permit one fresh explicit browser session under
the same exact pair. No CLI drain is permitted for this acceptance. On failed
fresh admission, unexpected identity/holder, detector ambiguity, unsafe state,
changed accounting/preservation, write/runtime failure, missing evidence or
unproved cleanup, stop effects, preserve the earliest boundary and leave the
task open. Fix/retest/publish before a new progress-backed ordinal; no unchanged
retry, new mining reservation, direct UART/pins, factory reset, arbitrary write
or speculative power manipulation. No human-response timeout is introduced.

Finish: confirm restored baseline and inactive lease, mine-on-boot false,
unchanged ledgers/preservation, browser streams/locks closed, supervisor reaped
and both USB nodes holder-free. Keep private evidence mode 0600 under the ignored
0700 root; committed evidence contains only validated closed counts/categories,
