...
Remaining blocker: The CPU0-affinity checkbox stays open only for measured main-telemetry cadence under hardware load. Affinity, priority, stack, handoff, readiness and cleanup are verified in software, but startup and Worker samples do not measure the telemetry loop's iteration/publication timing. Define bounded nonsecret timing instrumentation and explicit cadence acceptance criteria in a future complete contract, verify and publish it before any fresh hardware session. Do not equate the 500-ms sleep with a hard 500-ms period, reuse exhausted allowances, or relax safety deadlines. Migration, BWG restoration and STR-005 remain dependent on this proof.
...
Migration dependency: `task-fixed-usb-serial-qualification` remains open for CPU0 main-telemetry cadence under hardware load; [the 2026-09-13 reconciliation](docs/parity/evidence/20260913-usb-qualification-reconciliation.md) records the satisfied continuity, live-stop and Hello-recovery obligations. Qualification replaces TinyUSB handoff/recovery-006 requirements. The immutable legacy plan is historical and requires a successor contract before effects. Independent authorization, replay, monotonicity, protocol and evidence blockers remain unresolved.
