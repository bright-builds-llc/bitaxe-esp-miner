...
Remaining blocker: Bounded instrumentation and prospective criteria are implemented and software-verified, but preparation 2 could not collect idle cadence because Gate rejected review with `probe_admission`; USB-load and mining captures never started. Four exact-pair continuity cycles passed. [Cadence evidence](docs/parity/evidence/20260913-cpu0-telemetry-cadence.md) preserves the failure, unchanged 1200000-ms ledger / next ordinal 16, and complete host/device cleanup. Keep the CPU0-affinity/cadence checkbox open. The Gate admission correction is published as ad16c23d and pinned. Next: add a distinct evidence-preserving pre-mining failure closure/supersession path, then run four fresh exact-pair cycles and all three captures under unchanged limits. The permission-only unissued classifier and funded judge must continue rejecting this preparation. Migration, BWG restoration and STR-005 remain dependent on accepted cadence proof.
