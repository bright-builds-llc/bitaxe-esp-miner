...
- [ ] Verify/publish Gate, pin exact archive, verify/publish firmware and build the exact clean package.
- [ ] Fresh attempt-012 under the same protected campaign parent: cleanup/detect, state-preserving install, exact safe baseline and four current-image cycles with preservation and maximum exchanges.
- [ ] Use the qualified browser fan-only review: full-fan acknowledgement plus fresh post-command RPM, restored 30% only with fresh temperature at or below 45 C, and equal original ledger before/after. Do not issue a lease if this fails.
- [ ] Bind a version-2 successor to attempt-011's immutable ancestry/recovery/ledger and the new cooling/cycle receipts using `create-successor --predecessor-root <attempt-011> --input <budget-review> --cooling-input <cooling-review>`.
