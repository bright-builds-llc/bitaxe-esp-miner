...
- Run ID: `20260817T042626Z-STAT-001`
- Parity row: `STAT-001`
...
STAT-001 is the first actionable row. Attempt-011 stopped after 5 of 20
windows with sealed `watchdog_invalid_observation` at owner phase
`waiting_inbox`, while identity, safety, terminal state, cleanup, modes, and
seals passed. Exact pushed implementation
`0b5338f6c1224dbdae6e664cd286e114ad611c6c` changes that authoritative
boundary: the adapter now copies checkpoint/watchdog/phase observations before
sampling evaluation time, callers cannot supply stale time, and a controlled
regression proves old ordering is invalid while corrected ordering is
`feed_fresh` at age zero. The hashrate evaluator now binds 15 source paths.
Closure commit `cd5eea9a2d03e8d255907ed0a3f5502f48d24102` records every focused,
package, privacy, source, and mandatory gate as passing. Attempt-012 is
therefore a progress-backed test of a targeted correction, not an unchanged
retry.
...
protected roots, and real-child fixtures from consumed attempt-011 to fresh
attempt-012. Preserve campaign-result v13, network-continuity v7, owner-phase
vocabulary, 15-source evaluator, the public projection path, and every closed
watchdog, seal, category, precedence, redaction, and value-free behavior. Add
no firmware, mining, hashrate, sensor, watchdog-timeout, or hardware-control
behavior.
...
The profile domains remain distinct: ASIC frequency is 400 MHz, ASIC core
...
- [ ] Rebind roots, immutable plan/task admission, generated contract, Rust
      validator, Bazel inputs, and fixtures from attempt-011 to attempt-012
      without changing campaign behavior or the public projection path.
- [ ] Preserve v13/v7, all watchdog/owner-phase labels, 15-source identity,
      prior-ordinal rejection, seal/category gating, earliest-failure
      precedence, redaction, and value-free output across real child processes.
- [ ] Run every focused and mandatory software, firmware, privacy, reference,
      ...
      credential access.
- [ ] Execute only the frozen detector and conditional attempt-012 commands;
...
1. `test ! -e scratch/stat001-hashrate-monitor/wrapper-012 && (umask 077; mkdir -m 700 -p scratch/stat001-hashrate-monitor/wrapper-012 && just detect-ultra205 > scratch/stat001-hashrate-monitor/wrapper-012/detector.stdout 2> scratch/stat001-hashrate-monitor/wrapper-012/detector.stderr)`
2. Only after command 1 exits zero, admits exactly one Ultra 205 through
   ...
   `test ! -e scratch/stat001-hashrate-monitor/attempt-012 && test ! -e docs/parity/evidence/stat001-hashrate-monitor/hashrate-monitor-projection.json && test -s wifi-credentials.json && test -s pool-credentials.json && (umask 077; just capture-hashrate-monitor-evidence --private-root scratch/stat001-hashrate-monitor/attempt-012 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --pool-credentials pool-credentials.json --detector-output scratch/stat001-hashrate-monitor/wrapper-012/detector.stdout --projection docs/parity/evidence/stat001-hashrate-monitor/hashrate-monitor-projection.json --duration-seconds 600 --capture-timeout-seconds 1500 > scratch/stat001-hashrate-monitor/wrapper-012/capture.stdout 2> scratch/stat001-hashrate-monitor/wrapper-012/capture.stderr)`

The wrapper root must be mode `0700`; detector/capture streams must be distinct
mode-`0600` siblings; and the supervisor-owned attempt child, projection, and
candidate must be absent before launch. Starting command 2 consumes
attempt-012. Preserve the earliest typed failure and owner phase through
bounded safe stop, recovery, sealing, and cleanup. Never retry unchanged or
start attempt-013 under this plan. Stop on detector ambiguity or failure,
non-205 identity, missing credentials, source/reference/package drift, unsafe
state, incomplete quorum, safe-stop/recovery/cleanup/seal/mode/privacy failure,
nonzero command exit, or successful verified projection. Select one terminal
outcome: `complete`, `stop_repeated_boundary`, `stop_hardware_blocker`,
...
campaign network/watchdog/owner-phase/profile, generated-contract, real-child,
current-task/plan, source/reference admission, seal, protected-mode,
redaction, and failure-precedence tests. Run `just verify-redaction`, `just
verify-reference`, `just package`, and the mandatory sequence in order:
...
to prove board 205, attempt 12, exact source/reference/package/plan, one
