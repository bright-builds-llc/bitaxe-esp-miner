...
- Run ID: `20260814T044503Z-API-009`
- Parity row: `API-009`
- Initial status: `implemented`
- Source commit: `0fdff982c7c2914e1c455c6afd49fa95502f99e4`
- Reference commit: `c1915b0a63bfabebdb95a515cedfee05146c1d50`
- Active task: `task-parity-api009-command-effect-evidence-audit`
- Continues plan: `docs/parity/work-plans/20260814T042002Z-API-009/PLAN.md`

## Selection

The clean synchronized selector has no open plan and ranks API-009 first, so
no candidate is skipped. Attempt-015 proved exact-package flashing, runtime
identity, protocol admission, safe stop, and USB cleanup, then failed on one
malformed first serial candidate before 144 valid markers.

Pushed commit `0fdff982` reproduces the receive-ingress ownership gap and fixes
it with one allocation-free line admission at the USB session. Every ephemeral
reader open/reopen now discards its untrusted leading bytes through a newline,
then passes later chunks unchanged through the existing single callback fan-
out. Boundary, split-chunk, pass-through, reopen, source-ownership, strict
marker, device-session, flash, automation, real-process, mandatory, privacy,
reference, and firmware gates pass. This is new objectively verified host-
orchestration information, not an ordinal-only retry, so this plan authorizes
exactly one attempt-016 and no attempt-017.

## Scope and non-scope

Build and package exact clean pushed HEAD, admit exactly one Ultra 205 through
the protected detector, and run one fresh `attempt-016` through
...
mining for at most 600 seconds after the separately bounded activation phase;
...
voltage/frequency/fan override, non-205 device, attempt-017, or unchanged retry
is in scope.
...
- [ ] Commit and push this immutable plan/task checkpoint before package,
      ...
      effects.
- [ ] Re-run focused line-admission/startup-intent/timing/checkpoint/campaign/
      CLI/real-process tests, every mandatory software/privacy/reference gate,
      real firmware build, package validation, and exact clean-pushed identity
      admission.
- [ ] Require ignored `wifi-credentials.json` to be non-empty without reading
      it. Require fresh wrapper, detector, and attempt-016 paths plus the public
...
- [ ] Invoke exactly once:
      `just api-command-effects-campaign --private-root scratch/api009-command-effects/attempt-016 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --detector-output scratch/api009-command-effects/detector-016/detector.stdout --projection docs/parity/evidence/api009-command-effects/command-effects-projection.json --duration-seconds 600`.
- [ ] Consume `ready`, `rendered`, and `cleared` exactly once each only after
      ...
      command and the attempt-016 campaign root.
- [ ] Promote only on the complete independently validated command and restart
      ...
      withhold evidence, record cleanup/recovery, and stop without attempt-017.
...
Campaign start consumes attempt-016. Promote only if the closed projection
