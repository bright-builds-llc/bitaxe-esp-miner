...
- Run ID: `20260814T152151Z-API-009`
- Parity row: `API-009`
- Initial status: `implemented`
- Source commit: `5fb8f4cebcf6a906bd4d7494b86a6e405d4e9880`
- Reference commit: `c1915b0a63bfabebdb95a515cedfee05146c1d50`
- Active task: `task-parity-api009-command-effect-evidence-audit`
- Continues plan: `docs/parity/work-plans/20260814T143040Z-API-009/PLAN.md`

## Selection

The clean synchronized selector reports no open plan and ranks API-009 first.
Attempt-018 proved trusted exact-package runtime attestation, both admitted
flashes, the genuine notification, positive block count, pause and safe stop,
two IDENTIFY requests, and the rendered observation. Its earliest failure was
`safety_prerequisites_stale` after mining had been active throughout the
unbounded human observation windows. Deterministic source diagnosis reproduced
that consuming ready released the pause too early. Pushed source `5fb8f4ce`
keeps the single pause through ready, rendered, and cleared, resumes exactly
once afterward, confirms active recovery, and only then dismisses. Focused and
mandatory verification pass. This is objectively verified progress at the
exact failing boundary, not an ordinal-only retry, so this plan authorizes
exactly one attempt-019 and no attempt-020.

## Scope and non-scope

Build and package exact clean pushed HEAD, admit exactly one Ultra 205 through
the protected detector, and run one fresh attempt-019 through
...
waits safe-stopped for every private operator signal.
...
Signal `ready` only after a live typed required checkpoint exists and the user
reports they are watching. Signal `rendered` only when they report the exact
blank / `BITAXE IDENTIFY` / `Hello!` / blank frame while its live checkpoint
exists. Signal `cleared` only when they report that frame absent while its live
checkpoint exists.
...
fault injection, voltage/frequency/fan override, non-205 device, attempt-020,
...
- [ ] Commit and push this immutable plan/task checkpoint before package,
      ...
      effects.
- [ ] Re-run focused pause/signal/order/timing/checkpoint, campaign, CLI, and
...
- [ ] Require ignored `wifi-credentials.json` to be non-empty without reading
      it. Require fresh wrapper, detector, attempt-019, and public projection
...
- [ ] Invoke exactly once:
      `just api-command-effects-campaign --private-root scratch/api009-command-effects/attempt-019 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --detector-output scratch/api009-command-effects/detector-019/detector.stdout --projection docs/parity/evidence/api009-command-effects/command-effects-projection.json --duration-seconds 600`.
- [ ] Send each ready/rendered/cleared checkpoint exactly once with
      `just signal-api-command-identify --evidence-dir scratch/api009-command-effects/attempt-019/campaign --checkpoint <kind>` only after its matching live user report.
- [ ] Promote only on the complete independently validated command and restart
      ...
      withhold evidence, record cleanup/recovery, and stop without attempt-020.
...
Campaign start consumes attempt-019. Promote only if the closed projection
...
projection, and do not create attempt-020.
