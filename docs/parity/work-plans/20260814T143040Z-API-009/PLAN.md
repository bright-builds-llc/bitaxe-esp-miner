...
- Run ID: `20260814T143040Z-API-009`
- Parity row: `API-009`
- Initial status: `implemented`
- Source commit: `eacaea0fa9595a2bca60211af88370b9b655a69b`
- Reference commit: `c1915b0a63bfabebdb95a515cedfee05146c1d50`
- Active task: `task-parity-api009-command-effect-evidence-audit`
- Continues plan: `docs/parity/work-plans/20260814T123336Z-API-009/PLAN.md`

## Selection

Attempt-017 completed both admitted flashes but captured zero runtime-
attestation candidates before its typed `network_target_unavailable` stop.
Source investigation proved that synchronous ESP-IDF blocking Wi-Fi admission
preceded platform-readiness publication and could wait indefinitely for driver
events. Pushed source `16ee4ce8` moves storage, HTTP route-shell startup, and
recurring exact-package readiness before Wi-Fi admission; `eacaea0f` documents
the invariant. Focused and mandatory verification pass. This is objectively
verified progress at the exact failing boundary, not an ordinal-only retry, so
this plan authorizes exactly one attempt-018 and no attempt-019.

## Scope and non-scope

Build and package exact clean pushed HEAD, admit exactly one Ultra 205 through
the protected detector, and run one fresh attempt-018 through
...
and cleanup. Signal `ready` only after a live typed required checkpoint exists
and the user reports they are watching. Signal `rendered` only when they report
the exact blank / `BITAXE IDENTIFY` / `Hello!` / blank frame while its
30-second checkpoint is live. Signal `cleared` only when they report that frame
absent.
...
fault injection, voltage/frequency/fan override, non-205 device, attempt-019,
...
- [ ] Commit and push this immutable plan/task checkpoint before package,
      ...
      effects.
- [ ] Re-run focused startup-order, pause/signal/order/timing/checkpoint,
...
- [ ] Require ignored `wifi-credentials.json` to be non-empty without reading
      it. Require fresh wrapper, detector, attempt-018, and public projection
...
- [ ] Invoke exactly once:
      `just api-command-effects-campaign --private-root scratch/api009-command-effects/attempt-018 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --detector-output scratch/api009-command-effects/detector-018/detector.stdout --projection docs/parity/evidence/api009-command-effects/command-effects-projection.json --duration-seconds 600`.
- [ ] Send each ready/rendered/cleared checkpoint exactly once with
      `just signal-api-command-identify --evidence-dir scratch/api009-command-effects/attempt-018/campaign --checkpoint <kind>` only after its matching live user report.
- [ ] Promote only on the complete independently validated command and restart
      ...
      withhold evidence, record cleanup/recovery, and stop without attempt-019.
...
Campaign start consumes attempt-018. Promote only if the closed projection
...
projection, and do not create attempt-019.
