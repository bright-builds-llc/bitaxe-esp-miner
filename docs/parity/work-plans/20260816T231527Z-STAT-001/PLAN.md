...
- Run ID: `20260816T231527Z-STAT-001`
- Parity row: `STAT-001`
...
authority-blocked by the unfinished `BAP-001` UART/accessory lifecycle and the
absence of an authorized compatible accessory path.

`STAT-001` is the first actionable row. Consumed attempt-008 crossed exact
package and runtime identity, production serial, terminal transports and pool
state, safe stop, cleanup, seals, modes, redaction, and fourteen of twenty
required network windows before failing closed as `watchdog_unresponsive` with
the discriminator `watchdog_feed_stale`. Pushed correction commit
`f5f1be9b4614c155df96aaa78a2271c60065f84f` moves production-owner watchdog
feeds to completed session events and effects, the bounded inbox wait, and
campaign publication while preserving the compiled five-second timeout and
refusing to feed through an unfinished effect. Focused real-boundary tests and
the exact firmware package pass. Current pushed commit
`5c3d9f180e4a00a1799d1755fa7e9e9378462da3` adds only that correction's
truthful closure. Attempt-009 is therefore a source-progress-backed
continuation, not an unchanged retry.
...
heading was inventoried and complete priority blocks were loaded for safety,
privacy, security, authorization, evidence, retry, ESP-IDF ownership,
cross-process boundaries, operating-state units, watchdog participation,
transport qualification, evaluator identity, and hardware admission. The
following lower-priority blocks were not loaded: GSD frontmatter; native USB
prearmed capture; boot-proof replay lifetime; heartbeat/silent transport;
manual removal ownership; physical USB identity fields; cold-boot independent
observer; and ESP-IDF main-task runtime capacity. The 2026-08-03 audit baseline
has only six later lessons, is under 90 days old, and this work appends no
lesson, so no distinct audit trigger is due.
...
protected roots, and real-child fixtures from consumed attempt-008 to fresh
attempt-009. Preserve campaign-result v12, network-continuity v6, and the
...
it must never compare input voltage with the millivolt core setpoint. Require
fresh input voltage, power, ASIC temperature, and fan truth; at most 15 W;
ASIC temperature below 75 C; and fresh nonzero fan RPM after the 100-percent
fan command.
...
- [ ] Rebind roots, immutable plan/task admission, generated contract, Rust
      validator, Bazel inputs, and fixtures to attempt-009 without changing the
...
- [ ] Execute only the frozen detector and conditional attempt-009 commands;
...
1. `test ! -e scratch/stat001-hashrate-monitor/wrapper-009 && (umask 077; mkdir -m 700 -p scratch/stat001-hashrate-monitor/wrapper-009 && just detect-ultra205 > scratch/stat001-hashrate-monitor/wrapper-009/detector.stdout 2> scratch/stat001-hashrate-monitor/wrapper-009/detector.stderr)`
2. Only after command 1 exits zero, admits exactly one Ultra 205 through
   ...
   `test ! -e scratch/stat001-hashrate-monitor/attempt-009 && test ! -e docs/parity/evidence/stat001-hashrate-monitor/hashrate-monitor-projection.json && test -s wifi-credentials.json && test -s pool-credentials.json && (umask 077; just capture-hashrate-monitor-evidence --private-root scratch/stat001-hashrate-monitor/attempt-009 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --pool-credentials pool-credentials.json --detector-output scratch/stat001-hashrate-monitor/wrapper-009/detector.stdout --projection docs/parity/evidence/stat001-hashrate-monitor/hashrate-monitor-projection.json --duration-seconds 600 --capture-timeout-seconds 1500 > scratch/stat001-hashrate-monitor/wrapper-009/capture.stdout 2> scratch/stat001-hashrate-monitor/wrapper-009/capture.stderr)`
...
consumes attempt-009. Preserve the earliest typed failure and watchdog label
through bounded safe stop, recovery, sealing, and cleanup. Never retry
unchanged or start attempt-010 under this plan. Stop on detector ambiguity or
...
to prove board 205, attempt 9, exact source/reference/package/plan, one
