...
- Run ID: `20260813T180631Z-API-009`
- Parity row: `API-009`
- Initial status: `implemented`
- Source commit: `257e3be924ed4b08a80e29d7961ad010d5e1eeef`
- Reference commit: `c1915b0a63bfabebdb95a515cedfee05146c1d50`
- Active task: `task-parity-api009-command-effect-evidence-audit`
- Continues plan: `docs/parity/work-plans/20260813T174155Z-API-009/PLAN.md`

## Selection

The clean synchronized selector has no open plan and ranks API-009 first, so
no candidate is skipped. Attempt-012 admitted the package and USB session but
the parent killed its Rust campaign at 810 seconds, before the child's own
600-second observation plus 180-second terminal grace and outside-window USB
setup/cleanup could close. Pushed commit `257e3be9` replaces the fixed parent
and fixture deadlines with one checked budget covering the complete child
envelope, binds it to source limits, and preserves typed timeout recovery facts.

The scaled real-process cleanup regression, closed/missing/malformed timeout
tests, cross-language source contracts, focused targets, every mandatory gate,
and the real ESP firmware build pass. This is new objectively verified
production-orchestration information, not an ordinal-only retry, so this plan
authorizes exactly one attempt-013 and no attempt-014.

## Scope and non-scope

Build and package exact clean pushed HEAD, admit exactly one Ultra 205 through
the protected detector, and run one fresh `attempt-013` through
...
voltage/frequency/fan override, non-205 device, attempt-014, or unchanged retry
is in scope.
...
- [ ] Commit and push this immutable plan/task checkpoint before package,
      ...
      effects.
- [ ] Re-run focused budget/timeout/checkpoint/campaign/CLI/real-process tests,
...
- [ ] Require ignored `wifi-credentials.json` to be non-empty without reading
      it. Require fresh wrapper, detector, and attempt-013 paths plus the public
...
- [ ] Invoke exactly once:
      `just api-command-effects-campaign --private-root scratch/api009-command-effects/attempt-013 --package-manifest bazel-bin/firmware/bitaxe/bitaxe-ultra205-package.json --wifi-credentials wifi-credentials.json --detector-output scratch/api009-command-effects/detector-013/detector.stdout --projection docs/parity/evidence/api009-command-effects/command-effects-projection.json --duration-seconds 600`.
- [ ] Consume `ready`, `rendered`, and `cleared` exactly once each only after
      ...
      command and the attempt-013 campaign root.
- [ ] Promote only on the complete independently validated command and restart
      ...
      withhold evidence, record cleanup/recovery, and stop without attempt-014.
...
Campaign start consumes attempt-013. Promote only if the closed projection
