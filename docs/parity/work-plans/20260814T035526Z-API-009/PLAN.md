...
- Run ID: `20260814T035526Z-API-009`
- Parity row: `API-009`
- Initial status: `implemented`
- Source commit: `13f75265bb5439594bef95d69bebc0974705b5d9`
- Reference commit: `c1915b0a63bfabebdb95a515cedfee05146c1d50`
- Active task: `task-parity-api009-command-effect-evidence-audit`
- Continues plan: `docs/parity/work-plans/20260814T033105Z-API-009/PLAN.md`
...
identity, protocol admission, fixture lifecycle, and USB cleanup, then stopped
after the unchanged disabled boot preference reasserted a paused request two
milliseconds into the first active snapshot.

Pushed commit `13f75265` reproduces that production ownership edge and fixes it
with one lease-scoped current-boot run bootstrap after admitted command-effects
tracker construction and before owner readiness. Focused tests prove the first
active snapshot remains runnable, later explicit pause/resume commands remain
authoritative, consumed leases force pause, ordinary campaigns are unchanged,
and persisted mine-on-boot/NVS is not modified. Every mandatory gate and the
real ESP firmware build pass. This is new objectively verified production
information, not an ordinal-only retry, so this plan authorizes exactly one
attempt-015 and no attempt-016.

## Scope and non-scope

Build and package exact clean pushed HEAD, admit exactly one Ultra 205 through
the protected detector, and run one fresh `attempt-015` through
...
voltage/frequency/fan override, non-205 device, attempt-016, or unchanged retry
is in scope.
...
- [ ] Commit and push this immutable plan/task checkpoint before package,
      ...
      effects.
- [ ] Re-run focused startup-intent/timing/budget/checkpoint/campaign/CLI/
      real-process tests, every mandatory software/privacy/reference gate,
...
- [ ] Require ignored `wifi-credentials.json` to be non-empty without reading
      it. Require fresh wrapper, detector, and attempt-015 paths plus the public
...
- [ ] Consume `ready`, `rendered`, and `cleared` exactly once each only after
      ...
      command and the attempt-015 campaign root.
- [ ] Promote only on the complete independently validated command and restart
      ...
      withhold evidence, record cleanup/recovery, and stop without attempt-016.
...
Campaign start consumes attempt-015. Promote only if the closed projection
