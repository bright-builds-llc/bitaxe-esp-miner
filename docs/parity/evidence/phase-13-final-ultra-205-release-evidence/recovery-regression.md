...
`--allow-large-erase`, or `--allow-interrupted-ota`. Current helper behavior is
stricter than this blocked run: large erase also requires a fresh detector match
and immediate `board-info` preflight, and captured post-action conclusions
require the documented post-restore or post-interruption markers.
...
| destructive preflight | pending - allow flag not provided; a future run must rerun `just detect-ultra205`, require exactly one matching `port=`, and run immediate `espflash board-info` before erase |
...
| post-restore monitor markers | `capture_status=pending` in `recovery-regression/large-erase-post-restore-monitor.log`; future captured evidence must include `firmware_commit=`, `reference_commit=`, `safe_state: mining=disabled`, and `spiffs_mount=available` |
...
`DEVICE_URL`, the corresponding explicit allow flags, fresh destructive
preflights, and post-action operability markers are available.
