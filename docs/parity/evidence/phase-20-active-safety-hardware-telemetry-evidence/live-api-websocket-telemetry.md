...
device_url_source: missing - explicit DEVICE_URL env var was not available for this run
...
without a device URL. That path validates the `unsupported-pending` and
`deferred` manifest boundary, writes blocked evidence, and does not run `curl`,
scan the network, inspect ARP or mDNS state, or infer a target from redacted
serial output.
