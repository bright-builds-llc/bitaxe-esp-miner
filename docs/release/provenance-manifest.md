...
| --- | --- | --- |
| `esp-miner.bin` | Source commit, reference commit, generation command, offset, SHA-256, ESP-IDF version, linked component notice review | Package manifest evidence recorded in `docs/parity/evidence/phase-16-current-commit-release-evidence-completion.md`; firmware OTA upload, invalid rejection, rollback, and boot-validation remain blocked or pending; GPL-risk-reviewed release artifact; publication waits for final release approval. |
| `www.bin` | Static source tree, generation command, size, SHA-256, static asset provenance review | Package manifest evidence recorded in `docs/parity/evidence/phase-16-current-commit-release-evidence-completion.md`; live static, recovery, and OTAWWW whole-`www` evidence remains blocked or deferred; GPL-risk-reviewed release artifact; publication waits for final release approval. |
| `otadata-initial.bin` | ESP-IDF generated `ota_data_initial.bin` when available, otherwise an 8192-byte erased flash image generated by the package script | Package manifest evidence recorded in `docs/parity/evidence/phase-16-current-commit-release-evidence-completion.md`; rollback and boot-validation behavior remain below verified; GPL-risk-reviewed release artifact; publication waits for final release approval. |
| `bitaxe-ultra205-factory.bin` | Merge command, offsets, included artifacts, SHA-256, installation notes | Package manifest and detector-gated serial boot evidence recorded in `docs/parity/evidence/phase-16-current-commit-release-evidence-completion.md`; large erase restore evidence remains pending; GPL-risk-reviewed release artifact; publication waits for final release approval. |
| `bitaxe-ultra205-package.json` | Manifest v2 validation, artifact paths, checksums, tool versions, install notes, license/provenance links | Manifest-backed release-gate evidence recorded in `docs/parity/evidence/phase-16-current-commit-release-evidence-completion.md`; GPL-risk-reviewed release artifact; publication waits for final release approval. |
...
- Follow-up: run `tools/parity release-gate` after package output evidence is
  added and before any public release candidate is announced. Phase 16 package
  and serial evidence does not change the publication posture for distributed
  firmware images.
