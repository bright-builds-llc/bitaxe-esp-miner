# First Milestone: Foundation And Gamma 601 Bring-Up

The first milestone establishes the monorepo, build workflow, reference guardrails, and first hardware bring-up path. It does not attempt full mining parity.

## Goal

A developer can build the Rust firmware skeleton, package a flashable image, flash a connected Bitaxe Gamma 601 over USB, and observe a boot log from the Rust firmware.
...
- Minimal Gamma 601 firmware boot path that logs firmware identity and basic platform status.
...
- `just flash board=601 port=<port>` flashes the connected Gamma 601 or fails with a clear actionable error.
- `just monitor port=<port>` shows Rust firmware boot logs.
- `just flash-monitor board=601 port=<port>` flashes and monitors in one workflow.
...
- Hardware smoke note for Gamma 601 flash and boot log.
...
- BM1370 initialization parity beyond any stub or safe no-op needed for boot.
- Stratum networking.
- AxeOS API serving.
- OTA update behavior.
- Fan, voltage, power, and thermal control beyond safe platform initialization.
- Support claims for non-601 boards.
...
1. BM1370 ASIC protocol and safe initialization.
