...
- [x] Verify both repositories, publish Gate, pin its exact archive, publish
  firmware and build the exact clean ESP32-S3 package before hardware effects.
- [x] For each new runtime pair, prove cleanup/detection, admitted NVS-preserving
...
- [x] Preserve each first failure, recover/read retained diagnostics before
...
- [x] Complete a new exact-pair final campaign: normal up to 180000 ms with real work,
...
- [x] Verify no post-revocation work/submission/activation, qualified ordered
...
- [x] Verify/publish the changed pair and qualify four new cycles plus a fresh
...
- [x] Verify both changes, publish Gate first, pin it in firmware, publish and
...
- [x] Re-run required checks and package verification, publish any required
...
- [x] Add an immutable pre-admission rejection record and one exclusive
...
- [x] Bind the updated qualification driver to an exact pushed host commit;
...
- [x] Require unchanged recovered ledger/idle state, then fresh possession,
...
- [x] Test the denial matrix, source separation and successor-window binding;
  run required checks and publish this host-only recovery contract before effects.
- [x] Prepare and verify the observer first; then authorize/start promptly for
...
Completion review | 2026-09-08: Complete. The published diagnostic instrumentation,
targeted corrections, exact runtime qualification and independently authorized
successor final campaign passed. Firmware f3bbfd6a / Gate 2106f1c / driver 88ddb8a
remain the qualified identities. Normal ordinal 11 produced five correlated
accepted shares, three renewals and 59069 active ms; foreground ordinal 12's
exclusive continuation recorded 2807/2821-ms gate/shutdown timing; heartbeat
ordinal 13 recorded 2805/2826 ms. The final windows used 83599 active ms with
240000 ms fully charged. Separate ledger: 1140000 ms, next 14, last completed 13,
no pending entry. Original exhausted 240000-ms campaign remains unchanged.
The original unreserved ordinal-12 delivery and the child's earliest connection
failure remain preserved; no reservation refund or skipped ordinal occurred.

Current final evidence is safe_stop_complete, qualified fan_paused at 30%,
28 C, 3178 RPM, 0.44 W, 5.4775 V, mine_on_boot false, confirmed restoration and
inactive lease. Both owned tabs, serial ownership and listener closed; the
supervisor was reaped. Minimum observed owner stack was 7800 bytes. Stable
post-stop counters plus generation-guard regressions support the tested boundary;
sparse telemetry is not independent timing of every hardware operation.
Original panic causation and physical corrupt-receipt retention remain unproven.
Abrupt-loss stale-Hello recovery transfers to the active
`task-fixed-usb-hello-resynchronization`; this completed bounded campaign does
not resolve broader BWG, Stratum, soak or parity obligations.

Evidence: [final acceptance review](docs/parity/evidence/20260908-worker-preparation-live-acceptance.md).
All three sealed result validators and retained artifact snapshots passed
independent review. Source checks include ordered Rust gates, 114 harness tests,
93 canonical targets, real runtime packaging, Gate's 352 Rust/417 JavaScript
tests and required contract, ownership, provenance, privacy and standards checks.

Closing-record verification | 2026-09-08: ordered Cargo format, clippy, build
and tests passed again (2148 passed, one existing ignored). Redaction, reference,
fixed-USB ownership with ESP32-S3 packaging, parity validation, managed standards,
scoped report Markdown formatting and diff checks passed. Both task trackers
already fail whole-file formatter checks at the unchanged baseline; their native
...
Status: completed under the prospectively approved version-5 contract. Earlier
attempts 001–003 retain their original unverified outcomes. Successor to completed
...
- [x] Publish a versioned successor command contract separating deterministic
...
- [x] Complete source review, required ordered Cargo/browser/canonical checks,
  exact Gate publication/pin and native package verification before effects.
- [x] Run detector-admitted current-image continuity and bounded live recovery
...
- [x] Verify real work before loss, device-local revocation/shutdown, fresh
...
- [x] Seal original evidence, review redacted findings and report completion or
...
Completion review | 2026-09-11: Closed outcome `complete`. The published
6b6aa29e / Gate 632db811 pair passed all four fresh no-mining update/reconnect
cycles, maximum-size exchanges, diagnostic mining-loss recovery and separately
authorized resumed mining. Allowance 14 dispatched real work, released the
channel without Restore/Close traffic, and recovered on the first fresh Hello
without a CLI drain/reset/flash. Gate revocation was 2651 ms and shutdown began
at 2676 ms; retained TX-abandonment/epoch-revocation evidence supports link closure.
The post-work authorization checkpoint matched without resetting the original
baseline. Allowance 15 produced new work in generation 3 and safely stopped.

Both 30000-ms reservations remain charged. The qualification ledger is now next
ordinal 16, total 1200000 ms, last completed 15, pending false; the original
240000-ms campaign remains exhausted. Three ASIC nonce results were below the
pool target; no accepted share or renewal is claimed. The device is restored
with no active lease and mine-on-boot false. Both supervisors, owned processes,
listeners and serial holders cleared; the qualification tab closed explicitly.
The loss root was sealed before resume, and independent final verification bound
both immutable inventories and all 26 runtime-artifact copies.

The original unchecked no-mining stale-reply criterion above was superseded
prospectively, not retroactively passed: recovery skipped zero control replies,
so stale-reply hardware coverage remains false. Production-channel regressions
cover that branch separately. The active successor checklist is complete;
