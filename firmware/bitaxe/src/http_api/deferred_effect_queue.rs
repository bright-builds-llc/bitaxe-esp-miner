...
pub(super) struct DeferredEffectQueue<Effect> {
...
impl DeferredEffectLease {
    ...
    pub(super) fn release_after_response(self) -> Result<(), DeferredEffectQueueUnavailable> {
        self.release_sender
            .send(())
            .map_err(|_| DeferredEffectQueueUnavailable)
    }
...
pub(super) fn spawn_deferred_effect_worker<Effect, SpawnError>(
    capacity: usize,
