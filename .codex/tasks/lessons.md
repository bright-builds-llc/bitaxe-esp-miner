...
1. Date: 2026-09-04
2. What went wrong: TinyUSB Worker reserved its default 4096-byte internal FreeRTOS task stack before Wi-Fi startup, leaving the ESP32-S3 unable to satisfy a later 852-byte `DMA | 8BIT | INTERNAL` allocation; the allocator aborted directly and the board entered a panic reboot loop that looked like unstable USB enumeration. An attempted fix changed an unknown `TINYUSB_VENDOR_RX_BUFSIZE` symbol, so the compiled budget did not change and the exact failure repeated.
3. Preventive rule: Treat USB and Wi-Fi internal DMA-capable heap as one explicit budget. Pin the real TinyUSB task-stack symbol, assert its resolved ESP-IDF constant at compile time, fail on unknown Kconfig warnings or obsolete guessed symbols, keep logical Worker frames incrementally drained through the component's fixed full-speed FIFOs, and retain RTC-only allocation-failure evidence for field diagnosis.
4. Trigger signal to catch it earlier: A TinyUSB task or buffer allocation grows, generated sdkconfig omits a requested setting, the build logs an unknown Kconfig symbol, Worker enumerates briefly before repeated `panic` resets, or an allocation receipt reports capability mask `0x0000080c` during Wi-Fi startup.
