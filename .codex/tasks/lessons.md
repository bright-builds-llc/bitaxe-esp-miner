...
3. Preventive rule: Treat USB, Wi-Fi, and forced-internal pthread stacks as one ordered budget. Let Wi-Fi reserve its fixed resources before optional USB owners, preserve proven owner stack sizes, explicitly reserve enough internal memory from ordinary PSRAM-eligible `malloc` traffic, assert resolved Kconfig values at compile/build time, and retain pre-owner heap plus previous-boot failure evidence for field diagnosis.
