...
- [ ] Plan the next evidence seam from the one-shot result: passive `espflash` delivered zero connected-preflight bytes while the OS-native reader delivered valid same-session heartbeats; do not retry hardware ad hoc.
...
- [ ] Commit and push the complete software authority, then run exactly one OS-native cold qualification after the planned both-power recovery.
- [ ] If qualification passes, run one fresh qualified Plan 13 chain; otherwise persist the failure and plan an external UART or independent data channel without retrying.
