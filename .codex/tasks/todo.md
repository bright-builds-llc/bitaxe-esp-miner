...
- [x] Record the historical external-observer proposal after native-USB late attachment failed; this proposal was later superseded by the repo rule requiring non-invasive paths and fresh explicit authorization for direct UART or pin manipulation.
...
- [x] Implement the receive-only `uart-native` reader, physical/enumeration identity split, schema-v3 continuous qualification, and qualified formal Plan 13 integration without hardware use.
...
Completion review: The repository-wide policy, staged/CI redaction guard, stream-isolated dual flash capture, immutable private classifier input, explicit digest-bound software finalization, and Phase 35 private-input routing are implemented. Real-process regressions reproduce attempt 11's early-redaction origin loss, prove `direct_flash < classifier < finalize_evidence < original_read/PATCH`, and prove classifier rejection creates no admitted log or record and stops before mutation while cleanup remains secondary. Attempts 1–11 remain immutable; no detector, credentials, hardware, network, device request, mutation, reboot, admission, promotion, push, or attempt-12 authorization occurred. Full gate results are recorded in the quick-task summary.
