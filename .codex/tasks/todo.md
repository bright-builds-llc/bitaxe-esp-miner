...
- [x] Decide whether formal cold-start evidence needs an always-connected external UART/data-only arrangement or another machine-observable channel: use an independently powered receive-only UART observer.
...
- [ ] After the complete software authority is verified, committed, pushed, clean, and 0/0 with upstream, install the RX/GND-only fixture while both board power paths remain removed.
- [ ] Run exactly one schema-v3 qualification and, only if it passes, exactly one fresh formal Plan 13 closure chain.
...
Software update: Plan 14 now provides a standard-library receive-only UART reader, explicit UART0/USB console build contract, stable physical versus enumeration identity, a continuous external-UART lifecycle owner, strict post-boundary cold-session validation, and an exact-head/mode/owner/adapter/contract-bound schema-v3 qualification. Direct focused suites, all eight forced-uncached affected Bazel targets, the canonical ESP32-S3 build, reference/protected checks, shell/Perl/Python/Node hygiene, and the mandatory Rust sequence pass. Final commit, push, and clean 0/0 proof remain required before the fixture checkpoint.
