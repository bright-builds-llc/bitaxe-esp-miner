...
readonly script_dir
readonly smoke_script="${PHASE16_HTTP_STATIC_SMOKE_SCRIPT:-${script_dir}/phase16-http-static-smoke.sh}"

tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/phase16-http-static-smoke-test.XXXXXX")"
readonly tmp_root
...
case "${method} ${path}" in
  ...
  "GET /api/system/info")
    ...
    body="{\"firmware_commit\":\"26a1aebad7a1\",\"ssid\":\"HomeNetwork\",\"wifiPass\":\"secret\",\"stratumUser\":\"worker\",\"poolUrl\":\"stratum+tcp://pool.example:3333\",\"mac\":\"aa:bb:cc:dd:ee:ff\",\"ip\":\"192.168.1.5\"}"
    ;;
  "GET /api/phase16-unknown")
    status=404
    headers="Content-Type: application/json"
    body="{\"error\":\"unknown route\"}"
    ;;
  "GET /api/ws")
    status="${PHASE16_FAKE_WS_STATUS:-426}"
    headers="Content-Type: text/plain"
  ...
  "GET /api/ws/live")
    ...
    ;;
  "POST /api/system/OTA")
    status="${PHASE16_FAKE_OTA_STATUS:-500}"
    headers="Content-Type: text/plain"
    body="${PHASE16_FAKE_OTA_BODY:-Protocol Error}"
    ;;
...
test_missing_url_writes_blocker_without_curl() {
	...
	assert_contains "$log_file" "phase16_http_static_smoke"
...
test_invalid_url_writes_blocker_without_curl() {
	local out_dir="${tmp_root}/invalid-url"
	...
	CURL_BIN="$curl_stub" "$BASH" "$smoke_script" \
		--device-url "ftp://private-bitaxe.local" \
		--manifest "$manifest" \
		--out-dir "$out_dir"

	local log_file="${out_dir}/http-static-smoke.log"
	assert_contains "$log_file" "DEVICE_URL status: blocked - invalid DEVICE_URL scheme"
	assert_contains "$log_file" "DEVICE_URL sanitized: [invalid-url]"
	assert_contains "$log_file" "network_scan: disabled - using explicit DEVICE_URL only"
	assert_contains "$log_file" "http_static_status: blocked"
}

test_fake_success_records_required_phase16_routes() {
	...
	assert_contains "$log_file" "route: GET /phase16-missing-static"
	assert_contains "$log_file" "route: GET /recovery"
	assert_contains "$log_file" "route: GET /api/system/info"
	assert_contains "$log_file" "route: GET /api/phase16-unknown"
	...
	assert_contains "$log_file" "sanitized_target: http://[redacted]/api/system/OTA"
	assert_contains "$log_file" "actual_result: firmware_ota_validation_path"
	assert_contains "$log_file" "ota_route_presence: passed"
...
test_websocket_server_error_blocks_static_smoke() {
	...
	PHASE16_FAKE_WS_STATUS=500 CURL_BIN="$curl_stub" "$BASH" "$smoke_script" \
		--device-url "http://device.local" \
		--manifest "$manifest" \
		--out-dir "$out_dir"
...
test_ota_static_fallback_blocks_route_presence() {
	local out_dir="${tmp_root}/ota-static-fallback"
	local manifest="${tmp_root}/manifest-ota-static-fallback.json"
	local curl_stub="${tmp_root}/fake-curl-ota-static-fallback"

	create_manifest "$manifest"
	create_fake_curl "$curl_stub"

	PHASE16_FAKE_OTA_STATUS=404 PHASE16_FAKE_OTA_BODY="not found" CURL_BIN="$curl_stub" "$BASH" "$smoke_script" \
		--device-url "http://device.local" \
		--manifest "$manifest" \
		--out-dir "$out_dir"
	...
	local log_file="${out_dir}/http-static-smoke.log"
	assert_contains "$log_file" "route: POST /api/system/OTA"
	assert_contains "$log_file" "actual_status: 404"
	assert_contains "$log_file" "actual_result: unexpected_status"
	assert_contains "$log_file" "route_conclusion: blocked"
	assert_not_contains "$log_file" "ota_route_presence: passed"
}
...
test_curl_error_redacts_private_host() {
	...
	PHASE16_FAKE_CURL_STDERR_HOST=1 CURL_BIN="$curl_stub" "$BASH" "$smoke_script" \
		--device-url "http://device.local" \
		--manifest "$manifest" \
		--out-dir "$out_dir"
...
printf 'phase16_http_static_smoke_test passed\n'
