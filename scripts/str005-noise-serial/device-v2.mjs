import { ACT_TWO_BYTES, STAGES, OUTCOMES, boolean, canonical, cause, digest, object,
  port, privateIpv4, requireValue as check, uint } from "./contract-v2.mjs";

function release(value, job) {
  ...
  if (value.startedAtUs !== null && value.deadlineAtUs !== null) {
    check(value.deadlineAtUs === job.authorityDeadlineUs + 5_000_000, "noise_cleanup_deadline");
  }
...
function accepted(job) {
  ...
  const bounds = { noise_prepared: 60_000_000, tcp_connected: 5_000_000, act_one_written: 2_000_000,
    act_two_received: 10_000_000, proof_written: 2_000_000 };
...
export function parseNoiseStatusV2(value) {
  object(value, ["schema", "state", "observation", "job"]);
  check(value.schema === "worker-noise-diagnostic-status-v2" &&
  ...
  release(j.resources, j); stages(j);
...
export function validateNoiseProgressV2(previous, current) {
  const before = parseNoiseStatusV2(previous), after = parseNoiseStatusV2(current);
