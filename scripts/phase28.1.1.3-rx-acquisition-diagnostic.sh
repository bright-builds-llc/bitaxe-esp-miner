...
forced_ab_label="upstream_like_long_block_receive"
port=""
wifi_credentials=""
...
usage() {
	...
	printf '  Baseline RX-acquisition compare from Phase 28.1.1.1 source-work logs, then one\n'
	printf '  forced Rust A/B for investigation mode upstream_like_long_block_receive.\n'
}

die() {
	printf 'rx_acquisition_diagnostic_error: %s\n' "$*" >&2
	exit 1
}
...
poll_timeout_ms_observed() {
	local rust_log="$1"
	if [[ ! -f "$rust_log" ]]; then
		printf 'unavailable\n'
		return
	fi
	# Prefer the long-block value when present; otherwise last observed poll_timeout_ms.
	if rg -q 'poll_timeout_ms=10000' "$rust_log"; then
		printf '10000\n'
		return
	fi
	local observed
	observed="$(rg -o 'poll_timeout_ms=[0-9]+' "$rust_log" | tail -1 | sed 's/poll_timeout_ms=//' || true)"
	if [[ -n "$observed" ]]; then
		printf '%s\n' "$observed"
	else
		printf 'unavailable\n'
...
run_rx_acquisition_compare() {
	local upstream_log="$1"
	local rust_log="$2"
	local out_report="$3"
	node scripts/phase28.1.1.3-rx-acquisition-compare.mjs \
...
write_ab_report() {
	...
	local compare_report="$ab_dir/rx-acquisition-ab-compare-redacted.md"
	local ab_report="$ab_dir/28.1.1.3-ab-${forced_ab_label}-redacted.md"

	run_rx_acquisition_compare "$upstream_log" "$rust_log" "$compare_report"

	local result_correlated
	result_correlated="$(field_from_report result_correlated "$compare_report")"
	local partial_frame_rust
	partial_frame_rust="$(field_from_report partial_frame_rust "$compare_report")"
	local clear_rx_rust
	clear_rx_rust="$(field_from_report clear_rx_rust "$compare_report")"
	local rx_idle_rust
	rx_idle_rust="$(field_from_report rx_idle_rust "$compare_report")"
	local rx_complete_rust
	rx_complete_rust="$(field_from_report rx_complete_rust "$compare_report")"
	...
	local poll_timeout_observed
	poll_timeout_observed="$(poll_timeout_ms_observed "$rust_log")"
	...
	{
		printf '# Phase 28.1.1.3 A/B: upstream_like_long_block_receive\n\n'
		printf 'ab_label: %s\n' "$forced_ab_label"
		printf 'board: 205\n'
		...
		printf 'poll_timeout_ms_observed: %s\n' "$poll_timeout_observed"
		printf 'fake_pool_submit_observed: %s\n' "$ab_submit"
		printf 'result_correlated: %s\n' "$result_correlated"
		printf 'ab_outcome: %s\n' "$ab_outcome"
		printf 'partial_frame_rust: %s\n' "$partial_frame_rust"
		printf 'clear_rx_rust: %s\n' "$clear_rx_rust"
		printf 'rx_idle_rust: %s\n' "$rx_idle_rust"
		printf 'rx_complete_rust: %s\n' "$rx_complete_rust"
		printf 'result_read_attempt_rust: %s\n' "$result_read_rust"
		printf 'recommended_investigation: %s\n' "$recommended"
		printf 'phase30_promotion_input: pending\n'
		printf '\n## Notes\n\n'
		printf 'Forced investigation A/B for upstream-like long-block receive (RESULT_WORK_TIMEOUT_MS=10000).\n'
		printf 'Does not raise MAX_POLL_SLICE alone (SOCKET_TIMEOUT_MS clamps to 100).\n'
		printf 'Default firmware behavior unchanged; Plan 03 owns any default patch.\n'
	} >"$ab_report"

	copy_report_to_phase "$ab_report" "28.1.1.3-ab-${forced_ab_label}-redacted.md" >/dev/null
	printf '%s\n' "$ab_outcome"
}
...
write_blocked_evidence() {
	...
	mkdir -p "$phase_dir"
	{
		printf '# Phase 28.1.1.3 RX-Acquisition Diagnostic — Blocked\n\n'
		printf 'comparison_status: blocked_safe_prerequisite\n'
		printf 'blocker: %s\n' "$reason"
		printf 'recommended_investigation: result_rx_acquisition_model\n'
		printf 'partial_frame_rust: unavailable\n'
		printf 'clear_rx_rust: unavailable\n'
		printf 'rx_idle_rust: unavailable\n'
		printf 'rx_complete_rust: unavailable\n'
		printf 'fake_pool_submit_observed: false\n'
		printf 'result_correlated: false\n'
		printf 'raw_bytes_committed: false\n'
		printf 'credential_contents_read: false\n'
		printf 'phase30_promotion_input: pending\n'
	} >"$phase_dir/28.1.1.3-rx-acquisition-redacted.md"
	{
		printf '# Phase 28.1.1.3 A/B: upstream_like_long_block_receive\n\n'
		printf 'ab_label: upstream_like_long_block_receive\n'
		printf 'board: 205\n'
		printf 'port_selected: detector_gated\n'
		printf 'ab_capture_status: blocked_safe_prerequisite\n'
		printf 'poll_timeout_ms_observed: unavailable\n'
		printf 'fake_pool_submit_observed: false\n'
		printf 'result_correlated: false\n'
		...
		printf 'credential_contents_read: false\n'
		printf 'phase30_promotion_input: pending\n'
	} >"$phase_dir/28.1.1.3-ab-upstream_like_long_block_receive-redacted.md"
}
...
baseline_report="$capture_dir/rx-acquisition-redacted.md"
run_rx_acquisition_compare "$upstream_log" "$rust_log" "$baseline_report"
phase_baseline="$(copy_report_to_phase "$baseline_report" "28.1.1.3-rx-acquisition-redacted.md")"
...
printf 'rx_acquisition_diagnostic_status=complete\n'
