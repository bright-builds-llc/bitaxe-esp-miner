...
wrapper="${repo_root}/scripts/phase28.1.1.5-chip-enumerate-diagnostic.sh"
temp_root="$(mktemp -d "${repo_root}/scratch/phase28.1.1.5-chip-enumerate-test.XXXXXX")"
...
if bash "$wrapper" \
	...
	printf 'chip_enumerate_diagnostic_test_error: duration <360 must fail\n' >&2
	exit 1
fi
...
[[ "$combined" == *"forced_ab_label=count_asic_chips_rx_loop_parity"* ]]
[[ "$combined" == *"falsified_levers_recommended=false"* ]]
[[ "$combined" == *"read_chip_id_byte_patch_applied=false"* ]]
[[ "$combined" == *"job_byte_patch_applied=false"* ]]

# Forced A/B must be count_asic_chips_rx_loop_parity, not falsified prior labels.
rg -n "count_asic_chips_rx_loop_parity" "$wrapper" >/dev/null
...
if rg -n 'forced_ab_label="match_upstream_register_read_poll"|forced_ab_label=match_upstream_register_read_poll' "$wrapper"; then
	printf 'chip_enumerate_diagnostic_test_error: forced A/B must not be match_upstream_register_read_poll\n' >&2
	exit 1
fi
if rg -n 'forced_ab_label="post_max_baud_delay_2000"|forced_ab_label=post_max_baud_delay_2000' "$wrapper"; then
	printf 'chip_enumerate_diagnostic_test_error: forced A/B must not be post_max_baud_delay_2000\n' >&2
	exit 1
fi
if rg -n 'forced_ab_label="upstream_like_long_block_receive"|forced_ab_label=upstream_like_long_block_receive' "$wrapper"; then
	printf 'chip_enumerate_diagnostic_test_error: forced A/B must not be upstream_like_long_block_receive\n' >&2
	exit 1
fi
if rg -n 'forced_ab_label="ticket_mask_asic_difficulty"|forced_ab_label=ticket_mask_asic_difficulty' "$wrapper"; then
	printf 'chip_enumerate_diagnostic_test_error: forced A/B must not be ticket_mask_asic_difficulty\n' >&2
	exit 1
fi

printf 'phase28.1.1.5 chip-enumerate diagnostic helper tests passed\n'
