...
# post_max_baud / ticket_mask / chip-enumerate / work-field sole-blocker).
# Lever structural gate: mask_reload_tx_observed / post_configure_runtime after
# configure — NOT mask-value delta (fake-pool mask equals init 1fffe000).
set -euo pipefail
...
usage() {
	...
	printf '  Baseline ASIC mask-reload compare from Phase 28.1.1.1 source-work logs, then one\n'
	printf '  forced Rust A/B for pool_negotiated_mask_asic_reload (post-configure SetVersionMask).\n'
}

die() {
	printf 'asic_mask_reload_diagnostic_error: %s\n' "$*" >&2
	exit 1
}
...
if [[ "$dry_run" == "true" ]]; then
	...
	printf 'negotiated_version_mask_work_field_parity_reopened_as_sole_blocker=false\n'
	printf 'asic_mask_reload_candidate=true\n'
	exit 0
fi
...
run_mask_reload_compare() {
	local upstream_log="$1"
	local rust_log="$2"
	local out_report="$3"
	node scripts/phase28.1.1.7-asic-mask-reload-compare.mjs \
...
write_ab_report() {
	...
	local compare_report="$ab_dir/mask-reload-ab-compare-redacted.md"
	local ab_report="$ab_dir/28.1.1.7-ab-${forced_ab_label}-redacted.md"

	run_mask_reload_compare "$upstream_log" "$rust_log" "$compare_report"
	...
	local mask_reload_tx_observed
	mask_reload_tx_observed="$(field_from_report mask_reload_tx_observed "$compare_report")"
	...
	local wire_parity_mask_on_work_retained
	wire_parity_mask_on_work_retained="$(field_from_report wire_parity_mask_on_work_retained "$compare_report")"
	...
	[[ -n "$mask_reload_tx_observed" ]] || mask_reload_tx_observed="false"
	...
	[[ -n "$wire_parity_mask_on_work_retained" ]] || wire_parity_mask_on_work_retained="true"
	...
	[[ -n "$recommended" ]] || recommended="remaining_nonce_production_blocker_narrowing"
	...
	elif [[ "$mask_reload_tx_observed" != "true" ]]; then
		# Incomplete patch / blocked evidence — not hashing falsification (Pattern 4).
		ab_outcome="blocked_safe_prerequisite"
	...
	{
		printf '# Phase 28.1.1.7 A/B: pool_negotiated_mask_asic_reload\n\n'
		printf 'ab_label: %s\n' "$forced_ab_label"
		printf 'board: 205\n'
		...
		printf 'mask_reload_tx_observed: %s\n' "$mask_reload_tx_observed"
		...
		printf 'ab_outcome: %s\n' "$ab_outcome"
		printf 'wire_parity_mask_on_work_retained: %s\n' "$wire_parity_mask_on_work_retained"
		...
		printf '\n## Notes\n\n'
		printf 'Forced A/B for pool_negotiated_mask_asic_reload: runtime SetVersionMask after\n'
		printf 'configure + production_ready; init prelude×3 + init register retained; no job-byte OR.\n'
		printf 'Lever structural success: mask_reload_tx_observed + post_configure_runtime (D-05).\n'
		printf 'improved requires result_correlated and/or fake_pool_submit_observed (D-13).\n'
		printf 'Fake-pool mask equals init default — unchanged hashing after reload TX is valid falsification.\n'
		printf 'mask-on-MiningWork + ASIC-256 + RX-loop wire parity retained (D-15).\n'
	} >"$ab_report"

	copy_report_to_phase "$ab_report" "28.1.1.7-ab-${forced_ab_label}-redacted.md" >/dev/null
	printf '%s\n' "$ab_outcome"
}
...
run_rust_ab_capture() {
	...
	# Candidate is the runtime SetVersionMask reload patch already in tree (D-04/D-12).
	# No second lever; no job-byte patch.
...
write_blocked_evidence() {
	...
	mkdir -p "$phase_dir"
	{
		printf '# Phase 28.1.1.7 ASIC Mask-Reload Diagnostic — Blocked\n\n'
		printf 'comparison_status: blocked_safe_prerequisite\n'
		printf 'blocker: %s\n' "$reason"
		printf 'recommended_investigation: pool_negotiated_mask_asic_reload\n'
		printf 'forced_ab_label: %s\n' "$forced_ab_label"
		printf 'configure_observed: false\n'
		printf 'mask_stored_class: unavailable\n'
		printf 'mask_applied_to_work: false\n'
		printf 'mask_reload_tx_observed: false\n'
		...
		printf 'negotiated_version_mask_work_field_parity_reopened_as_sole_blocker: false\n'
		printf 'phase30_promotion_input: pending\n'
	} >"$phase_dir/28.1.1.7-mask-reload-redacted.md"
	{
		printf '# Phase 28.1.1.7 A/B: pool_negotiated_mask_asic_reload\n\n'
		printf 'ab_label: %s\n' "$forced_ab_label"
		printf 'board: 205\n'
		...
		printf 'wire_parity_mask_on_work_retained: true\n'
		...
		printf 'negotiated_version_mask_work_field_parity_reopened_as_sole_blocker: false\n'
		printf 'phase30_promotion_input: pending\n'
	} >"$phase_dir/28.1.1.7-ab-${forced_ab_label}-redacted.md"
}
...
baseline_report="$capture_dir/mask-reload-redacted.md"
run_mask_reload_compare "$upstream_log" "$baseline_rust_log" "$baseline_report"
phase_baseline="$(copy_report_to_phase "$baseline_report" "28.1.1.7-mask-reload-redacted.md")"
...
printf 'asic_mask_reload_diagnostic_status=complete\n'
