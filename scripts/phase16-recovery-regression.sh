...
usage() {
	cat >&2 <<'USAGE'
usage: phase16-recovery-regression.sh --manifest PATH --factory-image PATH --ota-image PATH --port PATH --out-dir PATH [--device-url URL] [--allow-failed-update] [--allow-large-erase] [--allow-interrupted-ota]
USAGE
}
...
http_static_smoke_script="${PHASE16_HTTP_STATIC_SMOKE_SCRIPT:-${script_dir}/phase16-http-static-smoke.sh}"
monitor_capture_script="${PHASE16_MONITOR_CAPTURE_SCRIPT:-${script_dir}/phase13-monitor-capture.sh}"
...
readonly failed_log="${out_dir}/failed-update.log"
...
manifest_field() {
	...
	if [[ ! -f "$manifest" || ! -x "$(command -v python3 2>/dev/null)" ]]; then
		printf 'unavailable'
		return
	fi
...
for artifact in data.get("artifacts", []):
    ...
    if os.path.basename(artifact_path) == expected_name:
        raise SystemExit(0)
...
run_http_static_smoke() {
	...
	if [[ ! -f "$http_static_smoke_script" ]]; then
		log_main "${label}_http_static_status: blocked - helper missing: ${http_static_smoke_script}"
		return 1
	fi

	log_main "${label}_http_static_command: scripts/phase16-http-static-smoke.sh --device-url [redacted] --manifest ${manifest} --out-dir ${out_path}"
...
post_restore_monitor_has_required_markers() {
	...
	for marker in \
		...
		if grep -Fq "$marker" "$monitor_log"; then
			log_both "$large_log" "post_action_safe_state_marker: ${marker} present"
			continue
		fi
...
require_phase16_live_action_gate() {
	local action_name="$1"
	local action_log="$2"
	local detector_log="${out_dir}/${action_name}-detect-ultra205.log"
	local board_info_log="${out_dir}/${action_name}-board-info.log"
	local detected_port
	local port_count
	...
	log_both "$action_log" "phase16_gate_status: running"
	log_both "$action_log" "phase16_gate_action: ${action_name}"
	log_both "$action_log" "phase16_gate_abort_conditions: detector mismatch, board-info failure, missing package artifacts, source_commit mismatch, missing restore image, missing OTA image"
	log_both "$action_log" "phase16_gate_recovery_steps: stop live action; restore with just flash board=205 when safe; capture monitor; rerun HTTP/static smoke when DEVICE_URL is provided"
	log_both "$action_log" "phase16_gate_post_action_safe_state_markers: firmware_commit=, reference_commit=, safe_state: mining=disabled, spiffs_mount=available"
	log_both "$action_log" "phase16_gate_detector_command: just detect-ultra205"

	set +e
	just detect-ultra205 >"$detector_log" 2>&1
	local detector_status=$?
	set -e
	log_both "$action_log" "phase16_gate_detector_output: ${detector_log}"
	log_both "$action_log" "phase16_gate_detector_status: ${detector_status}"
	if [[ "$detector_status" -ne 0 ]]; then
		log_both "$action_log" "phase16_gate_status: blocked - detector preflight failed"
		return 1
	fi
	...
	if [[ "$port_count" -ne 1 ]]; then
		log_both "$action_log" "phase16_gate_status: blocked - detector did not report exactly one port"
		return 1
	fi
	if [[ "$detected_port" != "$port" ]]; then
		log_both "$action_log" "phase16_gate_detected_port: ${detected_port}"
		log_both "$action_log" "phase16_gate_requested_port: ${port}"
		log_both "$action_log" "phase16_gate_status: blocked - detector port mismatch"
		return 1
	fi
	log_both "$action_log" "phase16_gate_detected_port: ${detected_port}"

	log_both "$action_log" "phase16_gate_board_info_command: espflash board-info --chip esp32s3 --port ${port} --non-interactive"
	set +e
	espflash board-info --chip esp32s3 --port "$port" --non-interactive >"$board_info_log" 2>&1
	local board_info_status=$?
	set -e
	log_both "$action_log" "phase16_gate_board_info_output: ${board_info_log}"
	log_both "$action_log" "phase16_gate_board_info_status: ${board_info_status}"
	if [[ "$board_info_status" -ne 0 ]]; then
		log_both "$action_log" "phase16_gate_status: blocked - board-info preflight failed"
		return 1
	fi

	for required_file in "$manifest" "$factory_image" "$ota_image"; do
		if [[ ! -f "$required_file" ]]; then
			log_both "$action_log" "phase16_gate_status: blocked - missing required file ${required_file}"
			return 1
		fi
	done

	manifest_board="$(manifest_field "image_metadata.board")"
	if [[ "$manifest_board" != "205" ]]; then
		log_both "$action_log" "phase16_gate_manifest_board: ${manifest_board}"
		log_both "$action_log" "phase16_gate_status: blocked - manifest board is not 205"
		return 1
	fi

	package_source_commit="$(manifest_field "source_commit")"
	local_source_commit="$(current_source_commit)"
	log_both "$action_log" "phase16_gate_manifest_source_commit: ${package_source_commit}"
	log_both "$action_log" "phase16_gate_current_source_commit: ${local_source_commit}"
	if [[ "$package_source_commit" != "$local_source_commit" ]]; then
		log_both "$action_log" "phase16_gate_status: blocked - manifest source_commit does not match current HEAD"
		return 1
	fi

	for artifact_name in bitaxe-ultra205-factory.bin esp-miner.bin; do
		if ! manifest_has_artifact_path "$artifact_name"; then
			log_both "$action_log" "phase16_gate_status: blocked - manifest missing artifact ${artifact_name}"
			return 1
		fi
		log_both "$action_log" "phase16_gate_artifact: ${artifact_name} present"
	done

	log_both "$action_log" "phase16_gate_status: passed"
}

write_common_header() {
	log_main "phase16_recovery_regression"
	...
	log_main "network_scan: disabled - using explicit DEVICE_URL only"
	log_main "raw_write_commands: prohibited"
	log_main "voltage_fan_mining_stress: prohibited"
	log_main "unbounded_fault_injection: prohibited"
	log_main "phase16_gate: required before failed-update, large-erase, and interrupted-update actions"
	log_main ""
}

run_failed_update() {
	if [[ "$allow_failed_update" -eq 0 ]]; then
		log_both "$failed_log" "failed_update_status: pending - allow flag not provided"
		return
	fi

	if [[ -z "$device_url" ]]; then
		log_both "$failed_log" "failed_update_status: blocked - missing DEVICE_URL"
		log_both "$failed_log" "failed_update_conclusion: blocked - failed-update live HTTP action not run"
		return 1
	fi

	require_phase16_live_action_gate "failed-update" "$failed_log"

	local invalid_image="${out_dir}/invalid-firmware.bin"
	printf 'phase16 invalid firmware image\n' >"$invalid_image"
	local checksum
	...
	log_both "$failed_log" "failed update route: POST /api/system/OTA"
	log_both "$failed_log" "failed update artifact: ${invalid_image}"
	log_both "$failed_log" "failed update artifact checksum: ${checksum}"
	log_both "$failed_log" "failed update failure point: invalid firmware upload to firmware OTA handler"
	log_both "$failed_log" "failed update curl_status: ${curl_status}"
	log_both "$failed_log" "failed update public status: ${status}"
	log_both "$failed_log" "failed update public body: $(body_snippet "$body")"
	if [[ -s "$error" ]]; then
		log_both "$failed_log" "failed update curl error: $(body_snippet "$error")"
	fi

	if [[ "$curl_status" -ne 0 ]]; then
		log_both "$failed_log" "failed_update_status: blocked - invalid-image request failed"
		return 1
	fi
	if [[ "$status" == "200" ]]; then
		log_both "$failed_log" "failed_update_status: blocked - invalid image was accepted"
		return 1
	fi
	if ! failed_update_status_is_rejection "$status"; then
		log_both "$failed_log" "failed_update_status: blocked - invalid image rejection returned unexpected status"
		return 1
	fi
	if ! failed_update_body_has_rejection_marker "$body"; then
		log_both "$failed_log" "failed_update_status: blocked - rejection body did not contain expected failure marker"
		return 1
	fi
	...
	if ! http_static_smoke_passed "$http_static_out"; then
		log_both "$failed_log" "failed_update_status: blocked - post-failure operability not proven"
		return 1
	fi

	log_both "$failed_log" "failed update post-failure partition/static/API state: HTTP/static smoke passed"
	log_both "$failed_log" "failed update recovery steps: explicitly not needed when invalid image is rejected and device remains reachable; otherwise restore factory image and capture monitor"
	log_both "$failed_log" "failed_update_status: captured"
}
...
run_large_erase() {
	...
	require_phase16_live_action_gate "large-erase" "$large_log"
	...
	if [[ -n "$device_url" ]]; then
		...
		log_both "$large_log" "large_erase_post_restore_http_static_status: pending - DEVICE_URL not provided"
	fi
...
run_interrupted_ota() {
	...
	require_phase16_live_action_gate "interrupted-ota" "$interrupted_log"
...
log_main "conclusion: pending - promote only redaction-cleared current-commit live evidence"
