...
import { successorRestartFixture, knownFailureDiagnostics, statisticsLine } from "./reset-origin-restart-successor-fixtures.mjs";
...
import {
  inspectNetworkFailureCapture,
  inspectRestartNetworkFailure,
  RESTART_NETWORK_FAILURE_SHA256,
  RESTART_NETWORK_FAILURE_PRODUCER,
} from "./reset-origin-restart-network-failure.mjs";
export async function failedNetworkRestartFixture(t) {
  ...
  const bytes = Buffer.from(
    (await readFile(capturePath, "utf8")).replaceAll("first_failure=none", "first_failure=network") +
  ...
  Object.assign(flash.fixed_serial_assessment, { startup_failed: true, issues: ["error_diagnostic", "startup_failed"] });
  ...
  const facts = inspectNetworkFailureCapture(bytes, f.context);
  const seal = {
    schema: "fixed-usb-restart-network-install-failed-inventory-v1",
    outcome: "unverified_network_startup_failure",
    context_sha256: digest(JSON.stringify(f.context)),
    auditor_sha256: RESTART_NETWORK_FAILURE_PRODUCER,
...
export async function networkRestartFixture(t, { preflight = true } = {}) {
  const old = await failedNetworkRestartFixture(t),
    nextCommit = "d".repeat(40),
  ...
  const input = resolve(old.base, "restart-network-progress.json");
  await writeNew(input, {
    schema: "worker-qualification-progress-v1",
    review: "verified",
    reason: "software_correction",
    evidence_sha256: [RESTART_NETWORK_FAILURE_SHA256],
  });
  const options = {
    ...old.options,
    input,
    privateRoot: resolve(dirname(old.root), "restart-3"),
  ...
  const operations = {
    ...
    readInstallFailure: async (root, ...args) => {
      ...
      const verified = await inspectRestartNetworkFailure(root, old.context);
      return { ...verified, binding: { root, failed_inventory_sha256: RESTART_NETWORK_FAILURE_SHA256 } };
    },
  };
