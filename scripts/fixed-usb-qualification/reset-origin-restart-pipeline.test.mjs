...
async function pipeline(
  ...
  t.after(() => closeServer());
  if (network || storage) successor = true;
  const f = await (storage
    ? storageRestartFixture(t)
    : network
      ? networkRestartFixture(t)
      : successor
        ? successorRestartFixture(t)
        : restartFixture(t));
  let clock = 100,
    state,
    config,
  ...
  const diagnosticNode = {
    get textContent() {
      if (successor && config.expectedFirmwareSourceCommit === f.context.before_source.firmware_commit) {
        ...
        if (badStorage) observations.find((value) => value.category === "storage_http_failure").error = "no_memory";
...
for (const { attempt, mode, failureOption } of [
  { attempt: 3, mode: "network", failureOption: "badNetwork" },
  { attempt: 4, mode: "storage", failureOption: "badStorage" },
]) {
  test(`attempt${attempt} real pipeline verifies known ${mode} failure then healthy install/capture/restart with unchanged accounting`, async (t) => {
    // Arrange
    const f = await pipeline(t, { [mode]: true });
    // Act
    await f.client.configureBeforeInstall();
    await f.worker.connect();
    await f.client.prepareInstallation();
    ...
    assert.equal(receipt.context.restart_attempt, attempt);
    assert.equal(receipt.ledger.next_ordinal, 17);
    assert.equal(receipt.ledger.total_charged_ms, 1380000);
    assert.equal(f.calls.filter((value) => value === "qualification_restart").length, 1);
  });
  test(`attempt${attempt} production client closes on changed ${mode} failure without consuming installation or returning restart authority`, async (t) => {
    // Arrange
    const f = await pipeline(t, { [mode]: true, [failureOption]: true });
    await f.client.configureBeforeInstall();
    await f.worker.connect();
    // Act / Assert
